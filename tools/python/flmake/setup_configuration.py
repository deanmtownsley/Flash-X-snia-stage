    ...
    def add_grid_interpolation(self):
        ...
        if gvars.gridInterpolation == setup_globals.GRID_INTERP_MONOTONIC:
            for unit_name in self.units.keys():
                if unit_name.startswith("Grid/GridMain/AMR/Paramesh4"):
                    # Check if added this already
                    for unit_name2 in self.units.keys():
                        if unit_name2.startswith("Grid/GridMain/AMR/Paramesh4/interpolation"):
                            break
                    else:
                        # add the required interpolation unit
                        ans.append("Grid/GridMain/AMR/Paramesh4/interpolation")
                    break
                elif unit_name.startswith("Grid/GridMain/AMR/Paramesh2"):
                    # Check if added this already
                    for unit_name2 in self.units.keys():
                        if unit_name2.startswith("Grid/GridMain/AMR/Paramesh2/monotonic"):
                            break
                    else:
                        # add the required interpolation unit
