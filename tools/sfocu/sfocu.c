...
/* declare function - defined in compareparticles.c*/
...
int sfocu(int argc, char **argv)
{
  ...
  int i, j, blockA, blockB, partIndexA, partIndexB, dim, failed, firstTime, a, b;
  ...
  */
  ...
  /*initialize particleReport*/
  particleReport particleReport = {0, 0, 0, 0, 0};
  ...
  else
  {
    if (A->totalparticles > 0 && B->totalparticles > 0)
    {
      ...
      compareparticles(&opts, A, B, pAllA, pAllB, pAtoB, &particleReport, particlesGotten);
  ...
  for (ivar = 0; ivar < report.nvar; ivar++)
  {
    ...
    for (blockA = 0; blockA < A->nblocks; blockA++)
    {
      ...
      for (i = 0; i < cellCount; i++)
      {
        ...
        report.maxA[ivar] = (blA->data[i] > report.maxA[ivar]) ? blA->data[i] : report.maxA[ivar];
        report.minA[ivar] = (blA->data[i] < report.minA[ivar]) ? blA->data[i] : report.minA[ivar];

        report.sumB[ivar] += blB->data[i]; /* FIXME (volume-weight?) */
        report.maxB[ivar] = (blB->data[i] > report.maxB[ivar]) ? blB->data[i] : report.maxB[ivar];
        report.minB[ivar] = (blB->data[i] < report.minB[ivar]) ? blB->data[i] : report.minB[ivar];
      }
