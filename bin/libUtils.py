...
import string, re, os, sys, subprocess, shutil
...
class LibUnion(dict):
   ...
   def makeBinary(self,libDir,libname,makefilename):
     ...
     * The file %(libDir)s/libinfo.py exists and contains a function 'libinfo'.
     ...
     * The file %(libDir)s/object/lib%(libname)s.a exists.
     * One of the files %(libDir)s/build.py or %(libDir)s/build.sh or %(libDir)s/build.csh exists.
       This build script is given the location of Makefile.h as its only argument.
       - If there is a libinfo.py, this build script must generate
         the library specified by return value of libinfo function.
       - Otherwise the build script must be executable, and must
         generate %(libDir)s/object/lib%(libname)s.a .
     """
     pwd = os.getcwd()
     os.chdir(libDir)
