...
contains
   ...
   function gamTau(i, j, tau)
      use mr_data, only: gamK, kmax
      implicit none
   ...
   function wTau(i, j, tau)
      use mr_data, only: wK, kmax
   ...
   subroutine gark_init()
      ...
      allocate (gamBar(mr_nstages_slow, mr_nstages_slow))
      allocate (wBar(mr_nstages_slow, mr_nstages_slow))

      gamBar = 0d0
      wBar = 0d0

      do k = 1, kmax
         gamBar = gamBar + gamK(:, :, k)/k
         wBar = wBar + wK(:, :, k)/k
      end do ! k
