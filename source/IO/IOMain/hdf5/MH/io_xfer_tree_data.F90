...
# endif
  ...
  integer, intent(IN) :: libType, xferType, localNumBlocks, &
       localOffset, presentDims, numFileBlks
...
#define c_loc(x) x
  ...
  integer :: typeMatchedXfer, err, tree_maxRefine
  logical :: doDataXfer
  ...
  if (doDataXfer) then
     ...
     call io_xfer_cont_slab(io_globalMe, fileID, libType, &
          ...
          (/localNumBlocks/), 1, c_loc(tree_data % nodetype(1)), (/numFileBlks/), err)
     ...
     call io_xfer_cont_slab(io_globalMe, fileID, libType, &
          ...
          (/localNumBlocks/), (/localOffset/), &
          (/localNumBlocks/), 1, c_loc(tree_data % lrefine(1)), (/numFileBlks/), err)
     IO_CHECK_XFER(err, lrefine_str)
     IO_TIMERS_STOP(lrefine_str)

     if (xferType == IO_READ_XFER .or. xferType == IO_READ_XFER_MASTER_PE) then
        !Abort if the max. refinement in restart is greater than than found in 
        ! checkpoint file block refinement
        !level 
        tree_maxRefine = maxval(tree_data % lrefine(1:localNumBlocks))
        if (tree_maxRefine > gr_maxRefine ) then
           write(6,'(2(a,i4))') &
                "WARNING! Max block refinement level in the checkpoint file ", &
                maxval(tree_data % lrefine(1:localNumBlocks)), &
                " is greater than runtime parameter lrefine_max:", gr_maxRefine
           call Logfile_stamp("WARNING! Refinement level in checkpoint "// &
                "file is greater than refinement level in restart file.")
        end if
     end if
  ...
  if (xferType == IO_WRITE_XFER .or. xferType == IO_WRITE_XFER_MASTER_PE) then
     ...
     call io_xfer_cont_slab(io_globalMe, fileID, libType, &
          ...
          (/numFileBlks, nfaces+nchild+1/), err)
     ...
     call io_xfer_cont_slab(io_globalMe, fileID, libType, &
          ...
          (/localNumBlocks/), (/localOffset/), &
          (/localNumBlocks/), 1, c_loc(tree_data % which_child(1)), (/numFileBlks/), err)
     ...
     if (libType == IO_FILE_HDF5) then
        ...
        call io_xfer_cont_slab(io_globalMe, fileID, libType, &
             ...
             (/localNumBlocks,MFLAGS/), 2, c_loc(tree_data % bflags(1,1)),& 
             (/numFileBlks, MFLAGS/), err)
     ...
     if ((xferType == IO_WRITE_XFER .or. xferType == IO_WRITE_XFER_MASTER_PE) .or. &
        ...
        call io_xfer_cont_slab(io_globalMe, fileID, libType, &
             ...
             (/localOffset,0,0,0,0/), &
             (/localNumBlocks,1+(K3D*2),1+(K2D*2),1+(K1D*2),2/), &
             5, c_loc(tree_data % gsurr_blks(1,1,1,1,1)), &
             (/numFileBlks,1+(K3D*2),1+(K2D*2),1+(K1D*2),2/), err)
        if (xferType == IO_READ_XFER .or. xferType == IO_READ_XFER_MASTER_PE) then
     ...
     if (xferType == IO_WRITE_XFER .or. xferType == IO_WRITE_XFER_MASTER_PE) then
        ...
        call io_xfer_cont_slab(io_globalMe, fileID, libType, &
             ...
             (/localNumBlocks/), (/localOffset/), &
             (/localNumBlocks/), 1, c_loc(tree_data % procnumber(1)), (/numFileBlks/), err)
     ...
     call io_xfer_cont_slab(io_globalMe, fileID, libType, &
          ...
          c_loc(tree_data % bnd_box(1,1,1)), (/numFileBlks, presentDims, 2/), err)
     ...
     call io_xfer_cont_slab(io_globalMe, fileID, libType, &
          ...
          c_loc(tree_data % coord(1,1)), (/numFileBlks, presentDims/), err)
     ...
     call io_xfer_cont_slab(io_globalMe, fileID, libType, &
          ...
          (/localNumBlocks,presentDims/), (/localOffset,0/), &
          (/localNumBlocks,presentDims/), 2, &
          c_loc(tree_data % bsize(1,1)), (/numFileBlks, presentDims/), err)
