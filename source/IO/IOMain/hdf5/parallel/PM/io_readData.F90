...
subroutine io_readData()
  ...
  use gr_specificData, ONLY : gr_nToLeft, gr_globaloffset, gr_gid, gr_gidIsValid
...
# endif
  ...
  call io_xfer_tree_data(tree_data, &
       ...
       localNumBlocks, localOffset, presentDims, gr_globalNumBlocks)

  !Extract data from gr_gid and gr_gsurr_blks arrays
  call MPI_ALLREDUCE(gr_is_gsurr_blks_initialized, isInitialized, 1,&
                     FLASH_LOGICAL, MPI_LAND, io_comm, ierr)
  gr_is_gsurr_blks_initialized = isInitialized
  call Grid_receiveInputData(localNumBlocks, alnblocks, xx)

  call MPI_ALLREDUCE(gr_gidIsValid, isValid, 1,&
                     FLASH_LOGICAL, MPI_LAND, io_comm, ierr)
  gr_gidIsValid = isValid
#ifdef FLASH_GRID_PARAMESH4DEV_SURR_BLKS_OPTION
  call MPI_ALLREDUCE(surr_blks_valid, isValid, 1,&
                     FLASH_LOGICAL, MPI_LAND, io_comm, ierr)
  surr_blks_valid = isValid
#endif
  call MPI_ALLREDUCE(gsurrblks_set, isSet, 1,&
                     FLASH_INTEGER, MPI_MIN, io_comm, ierr)
  gsurrblks_set = isSet
