...
#define offsetof(st, m) ((size_t) ( (char *)&((st *)(0))->m - (char *)0 ))
...
#ifdef FLASH_IO_ASYNC_HDF5
    dataset = H5Dcreate_async(*file_identifier, dataset_names[2], real_list_type,
...
#ifdef FLASH_IO_ASYNC_HDF5
      status = H5Dwrite_async(dataset, real_list_type, H5S_ALL, H5S_ALL,
  ...
  if (*num_int > 0) {
...
#ifdef FLASH_IO_ASYNC_HDF5
    dataset = H5Dcreate_async(*file_identifier, dataset_names[0], int_list_type,
                  dataspace, H5P_DEFAULT,H5P_DEFAULT,H5P_DEFAULT, io_es_id);
#else
    ...
    if ((*myPE == 0) || (*io_outputSplitNum != 1)) {
      /* write the data into the checkpoint file */    
#ifdef FLASH_IO_ASYNC_HDF5
      status = H5Dwrite_async(dataset, int_list_type, H5S_ALL, H5S_ALL,
  ...
  if (*num_str > 0) {
...
#ifdef FLASH_IO_ASYNC_HDF5
    dataset = H5Dcreate_async(*file_identifier, dataset_names[1], str_list_type,
                  dataspace, H5P_DEFAULT,H5P_DEFAULT,H5P_DEFAULT, io_es_id);
#else
    ...
    if ((*myPE == 0) || (*io_outputSplitNum != 1))  {
      /* write the data into the checkpoint file */  
#ifdef FLASH_IO_ASYNC_HDF5
      status = H5Dwrite_async(dataset, str_list_type, H5S_ALL, H5S_ALL,
  ...
  if (*num_log > 0) {
...
#ifdef FLASH_IO_ASYNC_HDF5
    dataset = H5Dcreate_async(*file_identifier, dataset_names[3], log_list_type,
                  dataspace, H5P_DEFAULT,H5P_DEFAULT,H5P_DEFAULT, io_es_id);
#else
    ...
    if ((*myPE == 0) || (*io_outputSplitNum != 1)) {
      /* write the data into the checkpoint file */    
#ifdef FLASH_IO_ASYNC_HDF5
      status = H5Dwrite_async(dataset, log_list_type, H5S_ALL, H5S_ALL,
