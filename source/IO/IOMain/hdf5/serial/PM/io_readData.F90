...
subroutine io_readData()
  ...
  use gr_specificData, ONLY : gr_gid, gr_gidIsValid, gr_nToLeft, gr_globalOffset
...
# endif
  ...
  if ((alnblocks > MAXBLOCKS .AND. xx /= 0) .OR. &
     ...
     call Driver_abort("[io_readData] ERROR: &
  ...
  if (io_globalMe == MASTER_PE) then
     ...
     do jproc=0, io_globalNumProcs -1
        ...
        call io_xfer_tree_data(tree_data, io_chkptFileID, IO_FILE_HDF5, &
             ...
             localNumBlockst, globalOffsett, presentDims, gr_globalNumBlocks)
