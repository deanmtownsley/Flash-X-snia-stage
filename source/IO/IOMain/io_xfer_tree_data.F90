  ...
  if (doDataXfer) then
     ...
     if (xferType == IO_READ_XFER .or. xferType == IO_READ_XFER_MASTER_PE) then
        ...
        if (any(tree_data % lrefine(1:localNumBlocks) > lrefine_max)) then
           ...
           call Driver_abort("At least 1 block in the checkpoint "// &
     ...
     call io_xfer_cont_slab(io_globalMe, fileID, libType, &
          ...
          (/localNumBlocks/), (/localOffset/), &
          (/localNumBlocks/), 1, c_loc(tree_data % which_child(1)), &
          (/numFileBlks/), err)

     if (err .ne. 0) then
        if (xferType == IO_READ_XFER .or. xferType == IO_READ_XFER_MASTER_PE) then
           if (xferType == IO_READ_XFER_MASTER_PE .OR. io_globalMe == MASTER_PE) &
                print *,"Warning: The 'which_child' array was not read successfully from the checkpoint file."
        else
     ...
     if (libType == IO_FILE_HDF5) then
        ...
        if (err .ne. 0) then
           if (xferType == IO_READ_XFER .or. xferType == IO_READ_XFER_MASTER_PE) then
              if (xferType == IO_READ_XFER_MASTER_PE .OR. io_globalMe == MASTER_PE) &
                   print *,"Note: The 'bflags' array was not read successfully from the checkpoint file."
           else
     ...
     if (xferType == IO_WRITE_XFER .or. xferType == IO_WRITE_XFER_MASTER_PE) then
        ...
        call io_xfer_cont_slab(io_globalMe, fileID, libType, &
             ...
             (/localNumBlocks/), (/localOffset/), &
             (/localNumBlocks/), 1, c_loc(tree_data % procnumber(1)), &
             (/numFileBlks/), err)
