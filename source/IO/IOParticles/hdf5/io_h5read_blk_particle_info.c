               ...
               int* global_offset)
{
  ...
  /* open the dataset */
#ifdef FLASH_IO_ASYNC_HDF5
  dataset = H5Dopen_async(*file_identifier, "blk particle info", H5P_DEFAULT, io_es_id);
#else
  dataset = H5Dopen(*file_identifier, "blk particle info", H5P_DEFAULT);
#endif  
  ...
  status = H5Dread_async(dataset, H5T_NATIVE_INT, memspace, dataspace, 
                H5P_DEFAULT, blk_particle_info, io_es_id);
#else
