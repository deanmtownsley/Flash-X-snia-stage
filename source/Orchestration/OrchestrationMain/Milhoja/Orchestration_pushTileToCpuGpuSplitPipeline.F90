...
subroutine Orchestration_pushTileToCpuGpuSplitPipeline(pktProto_Cptr, tileProto_Cptr, &
    ...
    use milhoja_runtime_mod, ONLY : milhoja_runtime_pushTileToCpuGpuSplitPipeline
#endif
...
#if ! defined(RUNTIME_MUST_USE_TILEITER) && defined(RUNTIME_SUPPORT_DATAPACKETS)
    CALL milhoja_runtime_pushTileToCpuGpuSplitPipeline(pktProto_Cptr, &
                                                  tileProto_Cptr, &
                                          MH_nThreads, MH_tileCInfo_Cp, MH_ierr)
    CALL Orchestration_checkInternalError("Orchestration_pushTileToCpuGpuSplitPipeline", MH_ierr)
#else
    CALL Driver_abort("Orchestration_pushTileToCpuGpuSplitPipeline: &
         &milhoja_runtime_pushTileToCpuGpuSplitPipeline disabled")
#endif
