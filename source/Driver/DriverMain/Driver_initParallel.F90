...
!FLASH_MPI2_OPENMP here really means: threading support requested, and assumed
!to be supported as per MPI 2 or greater.
...
#include "Simulation.h"
  ...
  integer, parameter :: mpiThreadLevelWanted = MPI_THREAD_MULTIPLE
#elif defined(_OPENMP) && defined(FLASH_MPI2_OPENMP)
  integer, parameter :: mpiThreadLevelWanted = MPI_THREAD_SERIALIZED
#endif
  ...
  call MPI_Init_thread(mpiThreadLevelWanted, iprovided, error)
...
#else
#ifdef FLASH_IO_ASYNC_HDF5
  ! use MPI_THREAD_MULTIPLE for HDF5 Async VOL
  call MPI_Init_thread(mpiThreadLevelWanted, iprovided, error)
  if (error /= MPI_SUCCESS) then
     print *, "Error from MPI_Init_thread for asynchronous HDF5 support"
     stop
  end if
#else
  call MPI_Init (error)
...
#if defined(_OPENMP) || defined(FLASH_IO_ASYNC_HDF5)
  if (dr_globalMe == 0) then

# if defined(FLASH_MPI2_OPENMP) || defined(FLASH_IO_ASYNC_HDF5)
     ...
     write(6,'(a,i3,a,i3)') " [Driver_initParallel]: "//&
          "Called MPI_Init_thread - requested level ", mpiThreadLevelWanted, &
          ", given level ", iprovided
     mpiThreadSupport = (iprovided >= mpiThreadLevelWanted);
# endif
