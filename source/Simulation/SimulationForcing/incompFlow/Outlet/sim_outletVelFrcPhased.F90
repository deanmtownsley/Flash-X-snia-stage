!!***if* source/Simulation/SimulationForcing/incompFlow/Outlet/sim_outletVelFrcPhased
!! NOTICE
...
subroutine sim_outletVelFrc2dPhased(vel, rhs, phi, xgrid, ygrid, &
   ...
   !---Local variables
   integer :: i, j, k, idimn, ibound, iforce, iliq, igas
   real :: xcell, ycell, velout, velforce
   real :: outprofile(LOW:HIGH, NDIM), velgrad(LOW:HIGH, NDIM), phiface(NDIM)
   ...
   do j = jy1, jy2
      do i = ix1, ix2
         ...
         phiface(:) = (/(phi(i, j, k) + phi(i - 1, j, k))*.5, &
...
subroutine sim_outletVelFrc3dPhased(vel, rhs, phi, xgrid, ygrid, zgrid, &
