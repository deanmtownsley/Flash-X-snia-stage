   ...
   do k = tileDesc%limits(LOW, KAXIS), tileDesc%limits(HIGH, KAXIS)
      do j = tileDesc%limits(LOW, JAXIS), tileDesc%limits(HIGH, JAXIS)
         do i = tileDesc%limits(LOW, IAXIS), tileDesc%limits(HIGH, IAXIS)
            ...
            vars(GXY_VAR, i, j, k) = 0d0 + 2d0*H*lx*ly
            vars(GXZ_VAR, i, j, k) = 0d0 + 2d0*H*lx*lz
            vars(GYY_VAR, i, j, k) = 1d0 + 2d0*H*ly*ly
            vars(GYZ_VAR, i, j, k) = 0d0 + 2d0*H*ly*lz
            vars(GZZ_VAR, i, j, k) = 1d0 + 2d0*H*lz*lz

            ! Extrinsic curvature (including 0 explicitly for ease of reading)
            vars(KXX_VAR, i, j, k) = 2d0*H*alp/r*(1d0 - (2d0 + H)*lx*lx)
            vars(KXY_VAR, i, j, k) = 2d0*H*alp/r*(0d0 - (2d0 + H)*lx*ly)
            vars(KXZ_VAR, i, j, k) = 2d0*H*alp/r*(0d0 - (2d0 + H)*lx*lz)
            vars(KYY_VAR, i, j, k) = 2d0*H*alp/r*(1d0 - (2d0 + H)*ly*ly)
            vars(KYZ_VAR, i, j, k) = 2d0*H*alp/r*(0d0 - (2d0 + H)*ly*lz)
            vars(KZZ_VAR, i, j, k) = 2d0*H*alp/r*(1d0 - (2d0 + H)*lz*lz)
         end do ! i
      end do ! j
   end do ! k
