!!****if* source/Simulation/SimulationMain/unitTest/Poisson/Amrex/Grid_unitTest
!! NOTICE
...
#include "Flashx_mpi.h"
  ...
  poisfact=1.
   call Grid_solveLaplacian(NSOL_VAR, RHS_VAR, COEF_FACE_VAR, bcTypes, bcValues, poisfact)
  call mpi_barrier(gr_meshComm,ierr)
  if (gr_meshMe .eq. 0) then
     CALL SYSTEM_CLOCK(TA(2),count_rate)
     ET=REAL(TA(2)-TA(1))/count_rate
     write(*,*) ' ' 
     write(*,*) 'Laplacian Solver time = ',ET,' sec.'
  endif
