     ...
     do kF = 0, kFmax, 2                                           ! loop over local k face indices
      ...
      Rsph = bndBoxILow + DeltaIHalf                              ! initial Rsph-axis location of i face
       ...
       do iC = imin, imax                                          ! loop over all radial cells
        ...
        xpm = Rsph * thetaSine * phiCosine - gr_mpolePtXpos
        ypm = Rsph * phiSine * thetaSine - gr_mpolePtYpos
        zpm = Rsph * thetaCosine - gr_mpolePtZpos
        
        rpm = sqrt(xpm*xpm + ypm*ypm + zpm*zpm)
...
!
        ...
        pointPotential = - gr_mpoleGravityConstant * point_mass/rpm
!
!
...
!
        ...
        if (j2 .and. k2) then
            solnData (ipotvar,iC,jC,kC-1) = solnData (ipotvar,iC,jC,kC-1) + facePotential + pointPotential
            solnData (ipotvar,iC,jC,kC) = solnData (ipotvar,iC,jC,kC) + facePotential + pointPotential
            solnData (ipotvar,iC,jC-1,kC) = solnData (ipotvar,iC,jC-1,kC) + facePotential + pointPotential
            solnData (ipotvar,iC,jC-1 ,kC-1) = solnData (ipotvar,iC,jC-1 ,kC-1) + facePotential + pointPotential
        else if (k2) then
