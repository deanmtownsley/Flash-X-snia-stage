...
subroutine Simulation_finalize()
  ...
  use Simulation_data, ONLY : sim_globalMe, sim_globalComm, &
                              sim_xmin, sim_xmax, &
                              sim_velx_i, sim_Sigma
  use rt_tm_interface, ONLY : rt_tm_reconstruction
...
#include "Flashx_mpi_implicitNone.fh"
  ...
  do while(itor%isValid())
     ...
     call ComputeError_SineWaveDiffusion( simTime * Second, MaxError_local )
  ...
  if ( sim_globalMe == MASTER_PE ) then
     WRITE(*,*)
     WRITE(*,'(A2,A)') '', 'INFO: SineWaveDiffusion Error'
     WRITE(*,*)
...
contains
  ...
  SUBROUTINE ComputeError_SineWaveDiffusion( t, MaxError )
    ...
    REAL(DP), PARAMETER :: A = 0.5, B = 0.49

    INTEGER  :: iE, iX1, iX2, iX3, iS
    INTEGER  :: iNodeZ, iNodeX1
    REAL(DP) :: X1, D_A, I1_A, I2_A, I3_A, V, L_x, Sigma, Tau

    V = sim_velx_i * Centimeter / Second

    L_x = ( sim_xmax - sim_xmin ) * Centimeter

    Sigma = sim_Sigma / Centimeter

    Tau = 3.0 * Sigma ! Diffusion Time Scale
    ...
    DO iE  = iE_B0   , iE_E0
      ...
      DO iNodeZ = 1, nDOFZ
        ...
        D_A  = A + B * SIN( TwoPi * ( X1 - V * t ) / L_x ) &
                     * EXP( - ( TwoPi / L_x )**2 * t / Tau )
        I1_A = - B * (TwoPi/L_x) / ( 3.0 * Sigma ) &
                   * COS( TwoPi * ( X1 - V * t ) / L_x ) &
                   * EXP( - ( TwoPi / L_x )**2 * t / Tau )
        I2_A = Zero
        I3_A = Zero
