...
subroutine Simulation_initBlock(solnData,tileDesc)
  ...
  use Grid_interface, ONLY : Grid_getCellCoords, Grid_getDeltas ,&
                             Grid_getGeometry, Grid_renormAbundance
...
#include "Multispecies.h"
  ...
  integer, dimension(LOW:HIGH,MDIM) :: blkLimits, blkLimitsGC !sneo
  integer :: blockID, level    !sneo
  ...
  ! sneo: changed in the input structure for Grid_getCellCoords
     ...
     do j = blkLimits(LOW,JAXIS),blkLimits(HIGH,JAXIS)
        do i = blkLimits(LOW,IAXIS),blkLimits(HIGH,IAXIS)
           ...
           if (radCenter <= sim_max_r .or. len_trim(progenitor_model_file) == 0) then
            ...
            !sneo:
            !print *, "radCenter, n1d_total, xzn(n1d_total): ",radCenter, n1d_total, xzn(n1d_total)
              velx_interp = 0.0
              vely_interp = 0.0
              velz_interp = 0.0
              do ivar = 1, NUNK_VARS
...
!For 'cutting out' the PNS using rigid body method
           ...
           if (radCenter <= sim_r_inner) then
              !call Grid_putPointData(blockId, CENTER, BDRY_VAR, EXTERIOR, rigid_axis, 1.0)
              solnData(BDRY_VAR,i,j,k) = 1.0   !sneo
              !call Grid_putPointData(blockId, CENTER, DENS_VAR, EXTERIOR, rigid_axis, 1.e-12)
              solnData(DENS_VAR,i,j,k) = 1.e-12
              !call Grid_putPointData(blockId, CENTER, TEMP_VAR, EXTERIOR, rigid_axis, 1.e3)
              solnData(TEMP_VAR,i,j,k) = 1.e3
              !call Grid_putPointData(blockId, CENTER, PRES_VAR, EXTERIOR, rigid_axis, 1.e-20)
              solnData(PRES_VAR,i,j,k) = 1e-20
              !call Grid_putPointData(blockId, CENTER, EINT_VAR, EXTERIOR, rigid_axis, 5.e18) ! eos_ModeInit is Dens Pres
              !call Grid_putPointData(blockId, CENTER, VELX_VAR, EXTERIOR, rigid_axis, 0.e0)!1.e-100)
              solnData(VELX_VAR,i,j,k) = 1.e-100
              !call Grid_putPointData(blockId, CENTER, VELY_VAR, EXTERIOR, rigid_axis, 0.e0)!1.e-100)
              solnData(VELY_VAR,i,j,k) = 1.e-100
              !call Grid_putPointData(blockId, CENTER, VELZ_VAR, EXTERIOR, rigid_axis, 0.e0)!1.e-100)
              solnData(VELZ_VAR,i,j,k) = 1.e-100
           else
              !call Grid_putPointData(blockId, CENTER, BDRY_VAR, EXTERIOR, rigid_axis, -1.0)
...
#include "Multispecies.h"
   ...
   error = abs(totalsum - 1.0)
      
   if (error > 0.01) then
           call Driver_abort("High error in abundance during initialization")
   end if
