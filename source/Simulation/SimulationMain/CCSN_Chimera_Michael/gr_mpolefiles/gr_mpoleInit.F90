!!****if* source/Simulation/SimulationMain/CCSN_Chimera_Michael/gr_mpolefiles/gr_mpoleInit
!!
!! NAME
!!
!!  gr_mpoleInit
...
!
  ...
  call RuntimeParameters_get  ("mpole_PtXpos",               gr_mpolePtXpos             )
  call RuntimeParameters_get  ("mpole_PtZpos",               gr_mpolePtZpos             )
  call RuntimeParameters_get  ("mpole_PtYpos",               gr_mpolePtYpos             )
  point_mass = point_mass + sim_pointMass
...
!
! sneo: gr_mpoleBlockList not needed anymore
!
!  allocate (gr_mpoleBlockList (1:MAXBLOCKS), stat = status)
!
!  if (status > 0) then
!      call Driver_abort ('[gr_mpoleInit] ERROR: gr_mpoleBlockList allocate failed')
!  end if
!
!
