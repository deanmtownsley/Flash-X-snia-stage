!!****if* source/Simulation/SimulationMain/CCSN_Chimera_Michael/gr_mpolePot2Dcylindrical
!! NOTICE
...
!
     ...
     zpm = bndBoxJLow - gr_mpolePtZpos                            !sneo: z-distance away from point mass
     do jF = 0, jFmax                                             ! loop over local j face indices
      ...
      xpm = bndBoxILow + iB * DeltaIHalf - gr_mpolePtXpos           !sneo: r-distance away from point mass
...
!
       ...
       if (rpm <= 0.0) then
            ...
            pointPotential = -gr_mpoleGravityConstant * point_mass/rpm   !sneo
       endif 
!
!
...
!
       if (i2) then
           ...
           solnData (ipotvar,iC-1,jC,1) = solnData (ipotvar,iC-1,jC,1) + facePotential + pointPotential 
           solnData (ipotvar,iC  ,jC,1) = solnData (ipotvar,iC  ,jC,1) + facePotential + pointPotential

       else if (j2) then

           solnData (ipotvar,iC,jC-1,1) = solnData (ipotvar,iC,jC-1,1) + facePotential + pointPotential
           solnData (ipotvar,iC,jC  ,1) = solnData (ipotvar,iC,jC  ,1) + facePotential + pointPotential
