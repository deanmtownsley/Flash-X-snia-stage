...
!
  ...
  if (abs (gr_mpoleTotalMass) < tiny (gr_mpoleTotalMass)) then
      call Driver_abortFlash ('[gr_mpoleCen3Dspherical] ERROR:  gr_mpoleTotalMass <= 0')
  end if
!
!
...
if (gr_mpoleDomainRmin == ZERO) then
  ...
  do while(itor%isValid())
     ...
     if (insideBlock) then
         ...
         call Grid_getCellCoords (IAXIS, FACES, lev, tileLimits(LOW,:), tileLimits(HIGH,:), shifts (1:nEdgesI))
         call Grid_getCellCoords (JAXIS, FACES, lev, tileLimits(LOW,:), tileLimits(HIGH,:), shifts (1:nEdgesJ))
         call Grid_getCellCoords (KAXIS, FACES, lev, tileLimits(LOW,:), tileLimits(HIGH,:), shifts (1:nEdgesK))
