!!****if* source/Simulation/SimulationMain/CCSN_Chimera_Michael/hy_rk_updateSoln
!! NOTICE
...
contains
 ...
 !!geometries.
   subroutine  geoFacs(i,j,k,facMx,facPx,facMy,facPy,facMz,facPz,Sgeo,U,V,dx,dy,dz)
     implicit none
     integer, intent(IN) :: i,j,k
     real, intent(OUT) :: facMx, facPx, facMy, facPy, facMz, facPz
     real, dimension(NFLUXES), intent(out) :: Sgeo
     real, intent(IN) :: U(:),dx,dy,dz
     ...
     Sgeo(HY_XMOM) = (V(DENS_VAR)*V(VELZ_VAR)*V(VELZ_VAR) + V(PRES_VAR)) / hy_xCenter(i)!T phi,phi
     Sgeo(MOM_PHI) =  V(DENS_VAR)*V(VELZ_VAR)*V(VELX_VAR) / hy_xCenter(i)!T phi,r

#ifdef SPARK_GLM
     ! P* is the total Pressure
     ! This presently does not work for POLAR coordinates
     Sgeo(HY_XMOM) = Sgeo(HY_XMOM) - (V(MAGZ_VAR)**2 - V(MAGP_VAR))/ hy_xCenter(i)
     ...
     Sgeo(HY_XMOM) = ((V(DENS_VAR)*V(VELZ_VAR)*V(VELZ_VAR) + V(PRES_VAR)) + &
             (V(DENS_VAR)*V(VELY_VAR)*V(VELY_VAR) + V(PRES_VAR))) / hy_xCenter(i)
     ! theta term: -(T_{R \theta} - Cot(\theta)*T_{\phi \phi})/R
     Sgeo(MOM_THT) = -((V(DENS_VAR)*V(VELX_VAR)*V(VELY_VAR)) - &
             (V(DENS_VAR)*V(VELZ_VAR)*V(VELZ_VAR) + V(PRES_VAR))*cotan(hy_yCenter(j))) / hy_xCenter(i)
...
subroutine update_solution(i,j,k,coeffs,dt, dtOld, dx, dy, dz, dhdt)
  ...
  real, pointer :: Fm(:), Fp(:), Gm(:), Gp(:), Hm(:), Hp(:)
  real :: facMx, facPx, facMy, facPy, facPz, facMz
  real, dimension(NFLUXES) :: Sgeo, Shy_grv, Stot
  real :: eint, ekin, emag
  real, pointer :: V0(:), Vstar(:)
  real, dimension(NFLUXES) :: U0, Ustar
  ...
  ! Get geometric factors and sources
   facMx = 1.0; facPx = 1.0; facMy = 1.0; facPy = 1.0; facMz=1.0; facPz=1.0; Sgeo = 0.0
   call geoFacs(i,j,k,facMx,facPx,facMy,facPy,facMz,facPz,Sgeo,Ustar,Vstar,dx,dy,dz)
  ...
  Ustar = coeffs(1)*U0 + coeffs(2)*Ustar +coeffs(3)*( &
        -dt/dx*(facPx*Fp-facMx*Fm) &
#if NDIM > 1
        -dt/dy*(facPy*Gp-facMy*Gm) &
#if NDIM ==3
        -dt/dz*(facPz*Hp-facMz*Hm) &
#endif
#endif
        +dt*Stot)
