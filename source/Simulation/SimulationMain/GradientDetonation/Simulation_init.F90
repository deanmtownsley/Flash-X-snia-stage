...
!! SYNOPSIS
!!
!!  Simulation_init(integer, INTENT(in)::myPE)
!!
!!
!! DESCRIPTION
!!
...
!! ARGUMENTS
!!
!!  myPE:        number id of local processor
!!
!!
!! PARAMETERS
!!
...
!!    ign_keep_pres      .true. for pertubation isobaric to upstream material
!!                       .false. for constant density perturbation
...
! -----------------------------------------------------------------------------------
      ...
      call Driver_abort("SORRY, Cellular not defined for non fixed block size")
...
!  real, dimension(q) :: xCoordsCell, yCoordsCell, zCoordsCell
  real             rvec(blockExtent(HIGH,IAXIS)*blockExtent(HIGH,JAXIS)*blockExtent(HIGH,KAXIS))
     ...
     call RuntimeParameters_get('xn14', sim_xn14)
