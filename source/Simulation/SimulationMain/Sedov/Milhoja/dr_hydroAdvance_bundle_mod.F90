...
contains
    ...
    !!
    !! @details
    !! Since this task function's dataItem is a tile, it can be written in 
    !! Fortran and use the Grid implementation's Fortran/C interoperability layer
    !! to access tile metadata.  Since this routine is called directly by a
    !! pthread running C++ code, it is part of the Task Function/Data Packet
    !! Fortran/C++ interoperability design and should satisy that design's
    !! requirements.  This justifies the fact that the interface exposes
    !! iso_c_binding types.
    !!
    !! @todo  Could the task function be a "fused task function" in
    !! the sense that it could run static Fortran code from different
    !! physics routines?
    !! @todo We are allocating/deallocating a scratch block for each block.  For
    !! this case, each thread in the team could have its own scratch block that
