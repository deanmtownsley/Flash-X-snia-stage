!!
!! Dean M. Townsley 2009-2018
!!  Spencer Caldwell 2018
!!
...
!!      2.0. impose specified max refinement (in fluff)
!!           determined by a density threshold
...
subroutine Grid_markRefineDerefine()
  ...
  use Grid_interface, ONLY : Grid_getBlkPtr, Grid_getListOfBlocks,&
       ...
       Grid_getBlkBoundBox
  use Simulation_data, ONLY : sim_refFluffDensThresh, sim_refFluffDensMargin, &
       sim_refFluffDensLevel, sim_refFluffThresh, sim_refFluffMargin, &
...
#include "Flash.h"
  ...
  real :: maxdens, minflff, maxenuc, maxfldt
...
!     ! below threshold force derefinement to sim_refFluffDensLevel
!     if ( maxdens < sim_refFluffDensThresh ) then
!        if ( lrefine(blkList(iblk)) > sim_refFluffDensLevel ) then
!           refine(blkList(iblk)) = .false.
!           derefine(blkList(iblk)) = .true.
!        else if ( lrefine(blkList(iblk)) == sim_refFluffDensLevel ) then
!           refine(blkList(iblk)) = .false.
!        endif
!     ! above threshold but within margin prevent refinement past FluffLevel
!     ! but don't force derefinement
!     else if ( maxdens < sim_refFluffDensThresh*(1.0+sim_refFluffDensMargin) &
!               .and. lrefine(blkList(iblk)) >= sim_refFluffDensLevel ) then
!        refine(blkList(iblk)) = .false.
!     endif
