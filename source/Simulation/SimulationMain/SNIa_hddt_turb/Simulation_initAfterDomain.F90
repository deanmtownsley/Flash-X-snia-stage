...
#include "mpif.h"
  ...
  real :: radius, dens, temp, xc12initial, xne20initial, xne22initial, f
     ...
     do k = 1, ksize
        ...
        do j = 1, jsize
           ...
           do i = 1, isize
              ...
              call sim_interpolate1dWd(radius, iCoords(2)-iCoords(1), dens, temp,&
              ...
              !if (temp < 1.0001*sim_vrms_T0) then
                 !f=0.0
              !else
                 !f = 1.0 formerly sim_vrms_reduced * ( (temp-0.9999*sim_vrms_T0)/sim_vrms_T0*2 )**sim_vrms_alpha (changed by JM)
              !endif

              !HERE GOES VEL LIMITS
              ! if(outside of sphere set vel=0)
               if (radius > sim_turbfield_bbox(IAXIS,HIGH) ) then
                  f = 0.0
               else
                  f = 8.0
               ! f = 1.0 for non-scaled and 8.0 for scaled version as per Nonaka
               endif
