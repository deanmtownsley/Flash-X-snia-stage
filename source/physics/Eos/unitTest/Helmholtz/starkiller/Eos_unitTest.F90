!!****if* source/physics/Eos/unitTest/Helmholtz/starkiller/Eos_unitTest
...
!!$        print*,ap,'Resulting Density
!min',minval(abs(solnData(DENS_VAR,ib:ie,jb:je,kb:ke)))
!!$        print*,ap,'Resulting Density
!max',maxval(abs(solnData(DENS_VAR,ib:ie,jb:je,kb:ke)))
  ...
  do while(itor%isValid())
     ...
     !  solnData(TEMP_VAR,ib:ie,jb:je,kb:ke)=0  ! don't zero TEMP or eos_helm
     !  cannot converge in MODE_DENS_EI
     solnData(PRES_VAR,:,:,:)=0 
  ...
  do while(itor%isValid())
     ...
     ! Now we have a "true"  temperature and internal energy; save them for
     ! comparison
     solnData(OPRS_VAR,ib:ie,jb:je,kb:ke)=solnData(PRES_VAR,ib:ie,jb:je,kb:ke)
     solnData(OENT_VAR,ib:ie,jb:je,kb:ke)=solnData(EINT_VAR,ib:ie,jb:je,kb:ke)
     solnData(OTMP_VAR,ib:ie,jb:je,kb:ke)=solnData(TEMP_VAR,ib:ie,jb:je,kb:ke)
