!!****if* source/physics/Hydro/HydroMain/unsplit/hy_updateSolution_fluxbuf
...
!! ARGUMENTS
!!
!!  tileDesc   - indentifies and describes the current block or tile
...
Subroutine hy_updateSolution_fluxbuf(tileDesc, flx,fly,flz,lo, Uin, Uout, del,timeEndAdv,dt,dtOld,sweepOrder,updateMode)
  ...
  use hy_interface,     ONLY : hy_unsplitUpdate,    &
                               hy_unitConvert,      &
                               hy_energyFix,        &
                               hy_putGravity
...
#include "UHD.h"
  ...
  real,   intent(IN),dimension(1:, lo(1): ,lo(2): ,lo(3): ) :: flx, fly, flz
  ...
  integer :: rangeSwitch ! can be set to one of UPDATE_ALL, UPDATE_INTERIOR, UPDATE_BOUND
...
!!$#endif
     ...
     if(.NOT. present(updateMode)) then
        rangeSwitch=UPDATE_ALL
!!$        call Grid_getFluxData(blockDesc,flx,fly,flz,datasize)
     else

        rangeSwitch = updateMode
     end if
     ...
     call hy_unsplitUpdate(tileDesc, Uin, Uout, rangeSwitch, &
                           dt, del, &
...
!!$             (blockID,rangeSwitch,blkLimits,dataSize,dt,del,flx,fly,flz, scrch_Ptr)
...
!!$     if (.not. blockNeedsFluxCorrect(blockID)) then
     ...
     if (rangeSwitch .NE. UPDATE_INTERIOR) then
