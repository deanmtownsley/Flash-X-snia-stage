...
#include "Eos.h"
  ...
  !$omp private(i,j,k,pos,eosData,massFraction,range)&
...
!   ! !  Begin loop over zones
!   ! !$omp do schedule(static) collapse(2)
!  ! !$omp target map(to:limits,i,k,range,vecLen,eos_eintSwitch,eos_smalle,eos_mapLookup,eos_gammam1,eos_gasConstant,eos_gamma,eos_singleSpeciesA,eos_singleSpeciesZ,eos_meshMe,eosData_size) map(alloc:eosData)
!   !$omp parallel if (hy_threadWithinBlock .AND. NDIM > 1) &
!   !$omp default(none) &
!   !$omp private(i,j,k)&
...
!   !$omp do collapse(3)
  ...
  !$omp shared(limits,eos_gammam1, eos_gasConstant, eos_gamma, eos_singleSpeciesA, eos_singleSpeciesZ, eos_mapLookup,eos_smalle,eos_eintSwitch,vecLen) &
  !$omp map(to:limits,eos_gammam1, eos_gasConstant, eos_gamma, eos_singleSpeciesA, eos_singleSpeciesZ, eos_mapLookup,eos_smalle,eos_eintSwitch,vecLen) &
