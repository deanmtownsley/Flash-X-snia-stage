!!****if* source/physics/Hydro/HydroMain/M1/hy_rk_getFaceFlux
!!
!!  NAME
!!
!!  hy_rk_getFaceFlux
!!
!!  SYNOPSIS
!!
!!  call hy_rk_getFaceFlux ( type(Grid_tile_t) :: blockDesc ) 
!!
!!  DESCRIPTION
!!  Initially stores grid data by 'pencils' (see subroutine setPencil() below),
!!  calls outside reconstruction subroutine, calls outisde Riemann solver, and 
!!  ultimately yields face fluxes for relevant directions.  These fluxes are lastly 
!!  saved to the grid multifabs (AMReX) for later access.
!!
!!  ARGUMENTS
!!  blockDesc-block/tile descriptor
!!  limits-sets limits of the fluxes.  Note this is a modified form typically yielded
!!         by blockDesc%limits b/c stage 1 of RK integration accounts for NSTENCIL 
!!         number of guard cells beyond the block interior.  Stage 2 only updates the 
!!         interior cells.
!!
!!***


!!****if* source/physics/Hydro/HydroMain/M1/hy_rk_getFaceFlux
!!
!!  NAME
!!
!!  hy_rk_getFaceFlux_offloaded
!!
!!  SYNOPSIS
!!
!!  call hy_rk_getFaceFlux_offloaded ( type(Grid_tile_t) :: blockDesc )
!!
!!  DESCRIPTION
!!  Initially stores grid data by 'pencils' (see subroutine setPencil() below),
!!  calls outside reconstruction subroutine, calls outisde Riemann solver, and 
!!  ultimately yields face fluxes for relevant directions.  These fluxes are lastly 
!!  saved to the grid multifabs (AMReX) for later access.
!!
!!  ARGUMENTS
!!  blockDesc-block/tile descriptor
!!  limits-sets limits of the fluxes.  Note this is a modified form typically yielded
!!         by blockDesc%limits b/c stage 1 of RK integration accounts for NSTENCIL 
!!         number of guard cells beyond the block interior.  Stage 2 only updates the 
!!         interior cells.
!!
!!***
!!!*** Already written in form of small function calls, so no need to insert kernel delimiters
!!Reorder(4):hy_starState,hy_fl[xyz]
subroutine hy_rk_getFaceFlux(limits,blkLimits,blkLimitsGC)

   use Hydro_data, ONLY : hy_threadWithinBlock, &
                          hy_starState, hy_grav, hy_flattening, hy_flx, hy_fly, hy_flz, &
                          hy_tposeBlk, hy_uplus, hy_uminus, hy_flat, hy_grv, hy_shk, hy_flux, &
                          hy_flat3d
   use Timers_interface, ONLY : Timers_start, Timers_stop
   use Driver_interface, ONLY : Driver_abort
 
   implicit none
 
#include "Simulation.h"
#include "constants.h"
#include "Spark.h"
#define NRECON HY_NUM_VARS+NSPECIES+NMASS_SCALARS
 
   integer, intent(IN), dimension(LOW:HIGH,MDIM) :: limits,blkLimits, blkLimitsGC
 
   integer :: i1,i2,i3, n, g, dir, ierr
   integer, dimension(3) :: guardCells
   integer, dimension(LOW:HIGH,MDIM) :: hy_lims
   integer :: i_global, j_global, k_global,v
   character(len = 2) :: dir_str

    
   !$omp target data map(to: dir, hy_lims, guardCells)

   if (hy_flattening) then
      !!!*** only needs transpiler
      call Driver_abort( "Flattening has not be implemented with GPU offloading nor has it been tested in FLASH5.")
      ! call flattening(hy_flat3d, limits)
   else
      ! call Timers_start("hy_flat3d")
      !$omp target teams distribute parallel do collapse(3) shared(blkLimitsGC,hy_flat3d) private(i1,i2,i3) default(none) map(to:blkLimitsGC)!! TODO: Set this once for both rk steps.
      do i3 = blkLimitsGC(LOW,KAXIS),blkLimitsGC(HIGH,KAXIS)
         do i2 = blkLimitsGC(LOW,JAXIS),blkLimitsGC(HIGH,JAXIS)
            do i1 = blkLimitsGC(LOW,IAXIS),blkLimitsGC(HIGH,IAXIS)
               hy_flat3d(i1,i2,i3) = 1.0
            enddo
         enddo
      enddo
      ! call Timers_stop("hy_flat3d")
   end if



   ! !  Begin loop over zones
   do dir = 1,  NDIM

      select case(dir)
      case (IAXIS)
         hy_lims(:,:) = limits(:,:)
         guardCells(1) = (limits(LOW,IAXIS) - blkLimitsGC(LOW,IAXIS))
         guardCells(2) = (limits(LOW,JAXIS) - blkLimitsGC(LOW,JAXIS))
         guardCells(3) = (limits(LOW,KAXIS) - blkLimitsGC(LOW,KAXIS))
         dir_str = "_x"
      case (JAXIS)
         hy_lims(:,1) = limits(:,JAXIS)
         hy_lims(:,2) = limits(:,IAXIS)
         hy_lims(:,3) = limits(:,KAXIS)
         guardCells(1) = (limits(LOW,JAXIS) - blkLimitsGC(LOW,JAXIS))
         guardCells(2) = (limits(LOW,IAXIS) - blkLimitsGC(LOW,IAXIS))
         guardCells(3) = (limits(LOW,KAXIS) - blkLimitsGC(LOW,KAXIS))
         dir_str = "_y"
      case (KAXIS)
         hy_lims(:,1) = limits(:,KAXIS)
         hy_lims(:,2) = limits(:,IAXIS)
         hy_lims(:,3) = limits(:,JAXIS)
         guardCells(1) = (limits(LOW,KAXIS) - blkLimitsGC(LOW,KAXIS))
         guardCells(2) = (limits(LOW,IAXIS) - blkLimitsGC(LOW,IAXIS))
         guardCells(3) = (limits(LOW,JAXIS) - blkLimitsGC(LOW,JAXIS))
         dir_str = "_z"
      end select 
      !$omp target update to(dir,guardCells,hy_lims)
      ! call Timers_start("hy_tposeBlk_init"//dir_str) 

      call initializeSnake(dir, hy_lims, guardCells) ! This is offloaded internally 


     !$omp target teams distribute parallel do collapse(4) &
     !$omp  private(i1,i2,i3,v) shared(hy_lims,guardCells,hy_uplus, hy_uminus, hy_tposeBlk, hy_flat) default(none)
     do i3 =1 + guardCells(3) , 1 + guardCells(3) + hy_lims(HIGH,3) - hy_lims(LOW,3)
        do i2 =1 + guardCells(2) , 1 + guardCells(2) + hy_lims(HIGH,2) - hy_lims(LOW,2)
           do i1 =1 + guardCells(1) - 1 , 1 + guardCells(1) + hy_lims(HIGH,1) - hy_lims(LOW,1) + 1
              do v=1,NRECON
                  call reconstruct(i1,i2,i3,v)
              enddo 
           enddo
        enddo
     enddo
   !   call reconstructOffloaded_split_inlined(hy_lims,guardCells)
      !$omp target teams distribute parallel do collapse(3) &
      !$omp private(i1,i2,i3,v) shared(hy_lims,dir,guardCells)
      do i3 = 1 + guardCells(3) , 1 + guardCells(3) + hy_lims(HIGH,3) - hy_lims(LOW,3)
         do i2 = 1 + guardCells(2) , 1 + guardCells(2) + hy_lims(HIGH,2) - hy_lims(LOW,2)
            do i1 = 1 + guardCells(1) - 1 , 1 + guardCells(1) + hy_lims(HIGH,1) - hy_lims(LOW,1) + 1
               call ensurePhysicalState(hy_uplus(:,i1,i2,i3))
               call ensurePhysicalState(hy_uminus(:,i1,i2,i3))
            enddo 
         enddo
      enddo


      ! call Timers_start("riemann"//dir_str)
      !  Now call the Riemann solver to compute hy_fluxes
      !$omp target teams distribute parallel do collapse(3) &
      !$omp private(i1,i2,i3) shared(hy_lims,dir,guardCells,hy_flux) default(none)
       do i3 = 1 + guardCells(3) ,1 + guardCells(3) + hy_lims(HIGH,3) - hy_lims(LOW,3)
          do i2 = 1 + guardCells(2) , 1 + guardCells(2) + hy_lims(HIGH,2) - hy_lims(LOW,2)
             do i1 = 1 + guardCells(1) , 1 + guardCells(1) + hy_lims(HIGH,1) - hy_lims(LOW,1) + 1
                 call riemann(i1,i2,i3,dir)   
            enddo
         enddo
      enddo
      !$omp target teams distribute parallel do collapse(3) &
      !$omp private(i1,i2,i3) shared(hy_lims,dir,guardCells,hy_flux) default(none)
      do i3 = 1 + guardCells(3) ,1 + guardCells(3) + hy_lims(HIGH,3) - hy_lims(LOW,3)
         do i2 = 1 + guardCells(2) , 1 + guardCells(2) + hy_lims(HIGH,2) - hy_lims(LOW,2)
            do i1 = 1 + guardCells(1) , 1 + guardCells(1) + hy_lims(HIGH,1) - hy_lims(LOW,1) + 1
               call avisc(i1,i2,i3,dir,NRECON)
               call mscalarFluxes(i1,i2,i3)
           enddo
        enddo
     enddo

      ! call Timers_stop("riemann"//dir_str)

       ! ***************
       ! Fluxes computed for one face of this zone
       ! Save the fluxes
       ! ***************
      !  call Timers_start("saveFluxes"//dir_str)

      !$omp target teams distribute parallel do collapse(3) &
      !$omp private(i1,i2,i3) shared(hy_lims,dir,guardCells) default(none)
       do i3 = 1 + guardCells(3) ,1 + guardCells(3) + hy_lims(HIGH,3) - hy_lims(LOW,3)
         do i2 = 1 + guardCells(2) , 1 + guardCells(2) + hy_lims(HIGH,2) - hy_lims(LOW,2)
            do i1 = 1 + guardCells(1) , 1 + guardCells(1) + hy_lims(HIGH,1) - hy_lims(LOW,1) + 1
               call saveFluxes(dir,i1,i2,i3,guardCells,hy_lims)
             enddo
          enddo
       enddo 
      !  call Timers_stop("saveFluxes"//dir_str)
      
    enddo
   !$omp end target data 
 contains
 
   !!Set loop dimensions based on the direction of the pencil set
   subroutine setLoop(dir)
      use Hydro_data, ONLY: hy_lims
     implicit none
     integer, intent(IN) :: dir
     !$omp declare target
     select case(dir)
     case (IAXIS)
        hy_lims(:,:) = limits(:,:)
     case (JAXIS)
        hy_lims(:,1) = limits(:,JAXIS)
        hy_lims(:,2) = limits(:,IAXIS)
        hy_lims(:,3) = limits(:,KAXIS)
     case (KAXIS)
        hy_lims(:,1) = limits(:,KAXIS)
        hy_lims(:,2) = limits(:,IAXIS)
        hy_lims(:,3) = limits(:,JAXIS)
     end select
 
   end subroutine setLoop
 
   subroutine saveFluxes(dir,i1,i2,i3,guardCells,hy_lims)
      USE Hydro_data, ONLY: hy_flux, hy_flx, hy_fly, hy_flz
     integer, intent(IN) :: dir,i1,i2,i3
     integer, dimension(3) :: guardCells
     integer, dimension(LOW:HIGH,MDIM) :: hy_lims
     integer :: i_global, j_global, k_global
     !$omp declare target
     i_global = -1 + i1 + hy_lims(LOW,1) - guardCells(1)
     j_global = -1 + i2 + hy_lims(LOW,2) - guardCells(2)
     k_global = -1 + i3 + hy_lims(LOW,3) - guardCells(3)
     select case(dir)
     case(IAXIS)
        hy_flx(:,i_global,j_global,k_global) = hy_flux(:,i1,i2,i3)
     case (JAXIS)
        hy_fly(:,j_global,i_global,k_global) = hy_flux(:,i1,i2,i3)
     case (KAXIS)
        hy_flz(:,j_global,k_global,i_global) = hy_flux(:,i1,i2,i3)
     end select
   end subroutine saveFluxes
 
!!Account for fluxes that are proportional to mass flux 
subroutine mscalarFluxes(i1,i2,i3)
   USE Hydro_data, ONLY: hy_flux, hy_uplus, hy_uminus
   implicit none
   real, pointer :: Xstar(:)
   integer, intent(IN) :: i1,i2,i3
   !$omp declare target
#if NSPECIES+NMASS_SCALARS==0
   return
#else
   if (hy_flux(HY_MASS,i1,i2,i3) > 0.) then
      Xstar => hy_uplus(HY_NUM_VARS+1:NRECON,i1-1,i2,i3)
   else
      Xstar => hy_uminus(HY_NUM_VARS+1:NRECON,i1,i2,i3)
   endif
   hy_flux(HY_NUM_FLUX+1:NFLUXES,i1,i2,i3) = Xstar*hy_flux(HY_MASS,i1,i2,i3)
#endif
end subroutine mscalarFluxes
 
 
!! Artificial viscosity as in Colella and Woodward
!! Calculation of cvisc here is different from the UHD solver
!! I am not including the transverse velocity terms to keep everything
!! neat and 1D for the pencil data
subroutine avisc(i1,i2,i3,dir,nvars) !flux,V,i1,nvars,dir)
   use Hydro_data, ONLY : hy_cvisc, hy_flux, hy_tposeBlk
   implicit none
   integer, intent(IN) :: nvars
   integer, intent(IN) :: i1,i2,i3,dir
   real :: cvisc, VenerLo, VenerHi
   !$omp declare target
   cvisc = hy_cvisc*max(-(hy_tposeBlk(HY_VELX+dir-1,i1,i2,i3) - hy_tposeBlk(HY_VELX+dir-1,i1-1,i2,i3)),0.)

   ! Construct minus and plus TOTAL energy densities
   VenerLo = hy_tposeBlk(HY_DENS,i1-1,i2,i3)*0.5*(dot_product(hy_tposeBlk(HY_VELX:HY_VELZ,i1-1,i2,i3),hy_tposeBlk(HY_VELX:HY_VELZ,i1-1,i2,i3)))&
            + hy_tposeBlk(HY_RHOE,i1-1,i2,i3)
   VenerHi = hy_tposeBlk(HY_DENS,i1,i2,i3)*0.5*(dot_product(hy_tposeBlk(HY_VELX:HY_VELZ,i1,i2,i3),hy_tposeBlk(HY_VELX:HY_VELZ,i1,i2,i3)))&
            + hy_tposeBlk(HY_RHOE,i1,i2,i3)

   hy_flux(HY_MASS:HY_ENER,i1,i2,i3) = &
         hy_flux(HY_MASS:HY_ENER,i1,i2,i3) &
         +cvisc*(/hy_tposeBlk(HY_DENS,i1-1,i2,i3)                 - hy_tposeBlk(HY_DENS,i1,i2,i3)&
         ,        hy_tposeBlk(HY_DENS,i1-1,i2,i3)*hy_tposeBlk(HY_VELX,i1-1,i2,i3) - hy_tposeBlk(HY_DENS,i1,i2,i3)*hy_tposeBlk(HY_VELX,i1,i2,i3)&
         ,        hy_tposeBlk(HY_DENS,i1-1,i2,i3)*hy_tposeBlk(HY_VELY,i1-1,i2,i3) - hy_tposeBlk(HY_DENS,i1,i2,i3)*hy_tposeBlk(HY_VELY,i1,i2,i3)&
         ,        hy_tposeBlk(HY_DENS,i1-1,i2,i3)*hy_tposeBlk(HY_VELZ,i1-1,i2,i3) - hy_tposeBlk(HY_DENS,i1,i2,i3)*hy_tposeBlk(HY_VELZ,i1,i2,i3)&
         ,        VenerLo                         - VenerHi/)
#ifdef SPARK_GLM
   hy_flux(HY_FMGX:HY_FPSI,i1,i2,i3) = &
         hy_flux(HY_FMGX:HY_FPSI,i1,i2,i3) &
         +cvisc*(/hy_tposeBlk(HY_MAGX,i1-1,i2,i3)                 - hy_tposeBlk(HY_MAGX,i1,i2,i3)&
         ,        hy_tposeBlk(HY_MAGY,i1-1,i2,i3)                 - hy_tposeBlk(HY_MAGY,i1,i2,i3)&
         ,        hy_tposeBlk(HY_MAGZ,i1-1,i2,i3)                 - hy_tposeBlk(HY_MAGZ,i1,i2,i3)&
         ,        hy_tposeBlk(HY_PSIB,i1-1,i2,i3)                 - hy_tposeBlk(HY_PSIB,i1,i2,i3)/)
#endif
end subroutine avisc
 
! !~ Flattening has not been tested yet in FLASH5, only 1D & 2D runs so far.
! subroutine flattening(hy_flat3d,limits)
!    !! This follows Miller & Colella 2002
!    use Hydro_data, ONLY : hy_starState
!    implicit none
!    integer, intent(IN), dimension(LOW:HIGH,MDIM) :: limits
!    !real, intent(OUT) :: hy_flat3d(GRID_IHI_GC,GRID_JHI_GC,GRID_KHI_GC)
!    !real :: flatTilde(NDIM,GRID_IHI_GC,GRID_JHI_GC,GRID_KHI_GC)
!    real, intent(OUT) :: hy_flat3d(xLoGC:xHiGC,yLoGC:yHiGC,zLoGC:zHiGC)
!    real :: flatTilde(NDIM,xLoGC:xHiGC,yLoGC:yHiGC,zLoGC:zHiGC)
!    real :: beta, Z
!    real, parameter :: betaMin = 0.75, betaMax = 0.85
!    real, parameter :: Zmin = 0.25, Zmax = 0.75
!    integer :: i,j,k, kx, ky, kz
!       pLo = blkLimitsGC(LOW,maxloc(blkLimitsGC(HIGH,1:MDIM)-blkLimitsGC(LOW,1:MDIM),1))
!    pHi = blkLimitsGC(HIGH,maxloc(blkLimitsGC(HIGH,1:MDIM)-blkLimitsGC(LOW,1:MDIM),1))
!    kx = 1
!    ky = 0
!    kz = 0
! #if NDIM>1
!    ky = 1
! #if NDIM==3
!    kz = 1
! #endif
! #endif
!    do k=limits(LOW,KAXIS)-kz, limits(HIGH,KAXIS)+kz
!       do j=limits(LOW,JAXIS)-ky, limits(HIGH,JAXIS)+ky
!          do i=limits(LOW,IAXIS)-kx, limits(HIGH,IAXIS)+kx
!             !1e-99 used to be TINY(1.0) but that produced Seg Faults
!             beta = abs(hy_starState(PRES_VAR,i+1,j,k)-hy_starState(PRES_VAR,i-1,j,k)) &
!                   / max(1e-99, abs(hy_starState(PRES_VAR,i+2,j,k)-hy_starState(PRES_VAR,i-2,j,k)))
!             Z    = abs(hy_starState(PRES_VAR,i+1,j,k)-hy_starState(PRES_VAR,i-1,j,k)) &
!                   / (hy_starState(GAMC_VAR,i,j,k)*hy_starState(PRES_VAR,i,j,k))
!             flatTilde(IAXIS,i,j,k) = max(0.,min(1.,(betaMax-beta)/(betaMax-betaMin)))
!             if (hy_starState(VELX_VAR,i+1,j,k)<hy_starState(VELX_VAR,i,j,k)) then
!                flatTilde(IAXIS,i,j,k) = max(flatTilde(IAXIS,i,j,k), &
!                      &                   min(1., (Zmax-Z)/(Zmax-Zmin)))
!             else
!                flatTilde(IAXIS,i,j,k) = 1.0
!             end if
! #if NDIM>1
!             beta = abs(hy_starState(PRES_VAR,i,j+1,k)-hy_starState(PRES_VAR,i,j-1,k)) &
!                   / max(1.e-99,abs(hy_starState(PRES_VAR,i,j+2,k)-hy_starState(PRES_VAR,i,j-2,k)))
!             Z    = abs(hy_starState(PRES_VAR,i,j+1,k)-hy_starState(PRES_VAR,i,j-1,k)) &
!                   / (hy_starState(GAMC_VAR,i,j,k)*hy_starState(PRES_VAR,i,j,k))
!             flatTilde(JAXIS,i,j,k) = max(0.,min(1.,(betaMax-beta)/(betaMax-betaMin)))
!             if (hy_starState(VELY_VAR,i,j+1,k)<hy_starState(VELY_VAR,i,j,k)) then
!                flatTilde(JAXIS,i,j,k) = max(flatTilde(JAXIS,i,j,k), &
!                      &                   min(1., (Zmax-Z)/(Zmax-Zmin)))
!             else
!                flatTilde(JAXIS,i,j,k) = 1.0
!             end if
! #if NDIM==3
!             beta = abs(hy_starState(PRES_VAR,i,j,k+1)-hy_starState(PRES_VAR,i,j,k-1)) &
!                   / max(1.e-99,abs(hy_starState(PRES_VAR,i,j,k+2)-hy_starState(PRES_VAR,i,j,k-2)))
!             Z    = abs(hy_starState(PRES_VAR,i,j,k+1)-hy_starState(PRES_VAR,i,j,k-1)) &
!                   / (hy_starState(GAMC_VAR,i,j,k)*hy_starState(PRES_VAR,i,j,k))
!             flatTilde(KAXIS,i,j,k) = max(0.,min(1.,(betaMax-beta)/(betaMax-betaMin)))
!             if (hy_starState(VELZ_VAR,i,j,k+1)<hy_starState(VELZ_VAR,i,j,k)) then
!                flatTilde(KAXIS,i,j,k) = max(flatTilde(KAXIS,i,j,k), &
!                      &                   min(1., (Zmax-Z)/(Zmax-Zmin)))
!             else
!                flatTilde(KAXIS,i,j,k) = 1.0
!             end if
! #endif
! #endif
!          end do
!       end do
!    end do
!    do k=limits(LOW,KAXIS)-kz, limits(HIGH,KAXIS)+kz
!       do j=limits(LOW,JAXIS)-ky, limits(HIGH,JAXIS)+ky
!          do i=limits(LOW,IAXIS)-kx, limits(HIGH,IAXIS)+kx
!             hy_flat3d(i,j,k) = minval(flatTilde(1:NDIM,i,j,k))
! #ifdef FLAT_VAR
!             hy_starState(FLAT_VAR,i,j,k) = hy_flat3d(i,j,k)
! #endif
!          end do
!       end do
!    end do
! end subroutine flattening
 
subroutine ensurePhysicalState(state)
   use Hydro_data, ONLY : hy_smalldens, hy_smallE, hy_smallpres, hy_smallX
   implicit none
   real, dimension(NRECON), intent(INOUT), target  :: state
   integer :: s
   real :: spcSumInv
   real, pointer :: spc(:)
   !$omp declare target
   state(HY_DENS) = max(hy_smalldens, state(HY_DENS))
   state(HY_PRES) = max(hy_smallpres, state(HY_PRES))
#if NSPECIES>0
   ! Limit and renormalize the species.
   spc => state(HY_NUM_VARS+1:HY_NUM_VARS+NSPECIES)
   do s = 1, NSPECIES
      spc(s) = max(hy_smallX,min(1.0,spc(s)))
   end do
   spcSumInv = 1./sum(spc(1:NSPECIES))
   spc = spc*spcSumInv
#endif
end subroutine ensurePhysicalState
 
 end subroutine hy_rk_getFaceFlux
 
 subroutine get_scratch_indices(hy_lims, i_new, j_new, k_new, i, j, k, guardCells)
   implicit none
   integer ,intent(in) :: i, j ,k
   integer, dimension(3) :: guardCells
   integer ,intent(in), dimension(LOW:HIGH,MDIM)  ::  hy_lims
   integer ,intent(OUT) :: i_new, j_new, k_new
   !$omp declare target
   i_new = 1 + i - hy_lims(LOW,1) + guardCells(1)
   j_new = 1 + j - hy_lims(LOW,2) + guardCells(2)
   k_new = 1 + k - hy_lims(LOW,3) + guardCells(3)
 
 end subroutine get_scratch_indices

 subroutine initializeSnake(dir, hy_lims, guardCells)
   use Hydro_data, only: hy_tposeBlk,hy_grv,hy_shk,hy_flat,hy_starState, hy_uplus, hy_uminus,hy_flat3d,hy_grav
implicit none
integer :: dir, l,i,j,k,n
integer, dimension(3) :: guardCells
integer :: i_s, j_s, k_s   ! Scratch indices
integer, dimension(LOW:HIGH,MDIM), intent(IN) :: hy_lims
!!$omp declare target

! Give hy_tposeBlk proper values
! Maps the hk_solnData to a hy_tposeBlk, such that it is contiguous in memory in the direction of iteration.
 !$omp target teams distribute parallel do collapse(3) default(none) shared(dir,hy_starState,hy_tposeBlk,hy_grv,hy_grav,hy_shk,hy_flat,hy_flat3d,hy_lims,guardCells) private(i,j,k,n,i_s, j_s, k_s)
   do k = hy_lims(LOW,3) - guardCells(3), hy_lims(HIGH,3) + guardCells(3)
      do j = hy_lims(LOW,2) - guardCells(2), hy_lims(HIGH,2) + guardCells(2)
         do i = hy_lims(LOW,1) - guardCells(1), hy_lims(HIGH,1) + guardCells(1)
            call get_scratch_indices(hy_lims,i_s,j_s,k_s,i,j,k, guardCells)
            if (dir == IAXIS) then
            hy_tposeBlk(HY_DENS,i_s,j_s,k_s) = hy_starState(DENS_VAR,i,j,k)
            hy_tposeBlk(HY_VELX,i_s,j_s,k_s) = hy_starState(VELX_VAR,i,j,k)
            hy_tposeBlk(HY_VELY,i_s,j_s,k_s) = hy_starState(VELY_VAR,i,j,k)
            hy_tposeBlk(HY_VELZ,i_s,j_s,k_s) = hy_starState(VELZ_VAR,i,j,k)
            hy_tposeBlk(HY_PRES,i_s,j_s,k_s) = hy_starState(PRES_VAR,i,j,k)
            hy_tposeBlk(HY_GAMC,i_s,j_s,k_s) = hy_starState(GAMC_VAR,i,j,k)
            hy_tposeBlk(HY_RHOE,i_s,j_s,k_s) = hy_starState(DENS_VAR,i,j,k)*hy_starState(EINT_VAR,i,j,k)
#ifdef SPARK_GLM
            hy_tposeBlk(HY_MAGX,i_s,j_s,k_s) = hy_starState(MAGX_VAR,i,j,k)
            hy_tposeBlk(HY_MAGY,i_s,j_s,k_s) = hy_starState(MAGY_VAR,i,j,k)
            hy_tposeBlk(HY_MAGZ,i_s,j_s,k_s) = hy_starState(MAGZ_VAR,i,j,k)
            hy_tposeBlk(HY_PSIB,i_s,j_s,k_s) = hy_starState(PSIB_VAR,i,j,k)
#endif
#if NSPECIES+NMASS_SCALARS>0
            do n=SPECIES_BEGIN, MASS_SCALARS_END
               hy_tposeBlk(HY_NUM_VARS+1+n-SPECIES_BEGIN,i_s,j_s,k_s)    = hy_starState(n,i,j,k)
            enddo
#endif
#ifdef GRAVITY
#ifdef GPOT_VAR
            hy_grv(i_s,j_s,k_s) = hy_starState(GPOT_VAR,i,j,k)
#else
            hy_grv(i_s,j_s,k_s) = hy_grav(IAXIS,i,j,k)
#endif
#endif
#ifdef SHOK_VAR
            hy_shk(i_s,j_s,k_s) = hy_starState(SHOK_VAR,i,j,k)
#else
            hy_shk(i_s,j_s,k_s) = 0.0
#endif
            hy_flat(i_s,j_s,k_s) = hy_flat3d(i,j,k)
            
else if (dir == JAXIS) then
            hy_tposeBlk(HY_DENS,i_s,j_s,k_s) = hy_starState(DENS_VAR,j,i,k)
            hy_tposeBlk(HY_VELX,i_s,j_s,k_s) = hy_starState(VELX_VAR,j,i,k)
            hy_tposeBlk(HY_VELY,i_s,j_s,k_s) = hy_starState(VELY_VAR,j,i,k)
            hy_tposeBlk(HY_VELZ,i_s,j_s,k_s) = hy_starState(VELZ_VAR,j,i,k)
            hy_tposeBlk(HY_PRES,i_s,j_s,k_s) = hy_starState(PRES_VAR,j,i,k)
            hy_tposeBlk(HY_GAMC,i_s,j_s,k_s) = hy_starState(GAMC_VAR,j,i,k)
            hy_tposeBlk(HY_RHOE,i_s,j_s,k_s) = hy_starState(DENS_VAR,j,i,k)*hy_starState(EINT_VAR,j,i,k)
#ifdef SPARK_GLM
            hy_tposeBlk(HY_MAGX,i_s,j_s,k_s) = hy_starState(MAGX_VAR,j,i,k)
            hy_tposeBlk(HY_MAGY,i_s,j_s,k_s) = hy_starState(MAGY_VAR,j,i,k)
            hy_tposeBlk(HY_MAGZ,i_s,j_s,k_s) = hy_starState(MAGZ_VAR,j,i,k)
            hy_tposeBlk(HY_PSIB,i_s,j_s,k_s) = hy_starState(PSIB_VAR,j,i,k)
#endif
#if NSPECIES+NMASS_SCALARS>0
            do n=SPECIES_BEGIN, MASS_SCALARS_END
               hy_tposeBlk(HY_NUM_VARS+1+n-SPECIES_BEGIN,i_s,j_s,k_s)    = hy_starState(n,j,i,k)
            enddo
#endif
#ifdef GRAVITY
#ifdef GPOT_VAR
            hy_grv(i_s,j_s,k_s) = hy_starState(GPOT_VAR,j,i,k)
#else
            hy_grv(i_s,j_s,k_s) = hy_grav(IAXIS,j,i,k)
#endif
#endif
#ifdef SHOK_VAR
            hy_shk(i_s,j_s,k_s) = hy_starState(SHOK_VAR,j,i,k)
#else
            hy_shk(i_s,j_s,k_s) = 0.0
#endif
            hy_flat(i_s,j_s,k_s) = hy_flat3d(j,i,k)

else if (dir == KAXIS) then
            hy_tposeBlk(HY_DENS,i_s,j_s,k_s) = hy_starState(DENS_VAR,j,k,i)
            hy_tposeBlk(HY_VELX,i_s,j_s,k_s) = hy_starState(VELX_VAR,j,k,i)
            hy_tposeBlk(HY_VELY,i_s,j_s,k_s) = hy_starState(VELY_VAR,j,k,i)
            hy_tposeBlk(HY_VELZ,i_s,j_s,k_s) = hy_starState(VELZ_VAR,j,k,i)
            hy_tposeBlk(HY_PRES,i_s,j_s,k_s) = hy_starState(PRES_VAR,j,k,i)
            hy_tposeBlk(HY_GAMC,i_s,j_s,k_s) = hy_starState(GAMC_VAR,j,k,i)
            hy_tposeBlk(HY_RHOE,i_s,j_s,k_s) = hy_starState(DENS_VAR,j,k,i)*hy_starState(EINT_VAR,j,k,i)
#ifdef SPARK_GLM
            hy_tposeBlk(HY_MAGX,i_s,j_s,k_s) = hy_starState(MAGX_VAR,j,k,i)
            hy_tposeBlk(HY_MAGY,i_s,j_s,k_s) = hy_starState(MAGY_VAR,j,k,i)
            hy_tposeBlk(HY_MAGZ,i_s,j_s,k_s) = hy_starState(MAGZ_VAR,j,k,i)
            hy_tposeBlk(HY_PSIB,i_s,j_s,k_s) = hy_starState(PSIB_VAR,j,k,i)
#endif
#if NSPECIES+NMASS_SCALARS>0
            do n=SPECIES_BEGIN, MASS_SCALARS_END
               hy_tposeBlk(HY_NUM_VARS+1+n-SPECIES_BEGIN,i_s,j_s,k_s)    = hy_starState(n,j,k,i)
            enddo
#endif
#ifdef GRAVITY
#ifdef GPOT_VAR
            hy_grv(i_s,j_s,k_s) = hy_starState(GPOT_VAR,j,k,i)
#else
            hy_grv(i_s,j_s,k_s) = hy_grav(IAXIS,j,k,i)
#endif
#endif
#ifdef SHOK_VAR
            hy_shk(i_s,j_s,k_s) = hy_starState(SHOK_VAR,j,k,i)
#else
            hy_shk(i_s,j_s,k_s) = 0.0
#endif
            hy_flat(i_s,j_s,k_s) = hy_flat3d(j,k,i)
         endif

         enddo
      enddo
   enddo



end subroutine initializeSnake


#include "reconstruct.F90"
#include "riemann.F90"
