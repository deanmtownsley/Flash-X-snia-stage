...
!! remember that we use the logs of density
  ...
  if (lca.eq.0) then !! fail gracefully
     lca = klt
     !DON: Don't interpolate to rc12 = 0.0
     !rc12 = 0.0
  ...
  if (lna.eq.0) then !! fail gracefully
     lna = jlt
     !DON: This is okay because the minimum rne22 in the table is 0.0
     rne22 = 0.0
  else if (lna.eq.nne22it) then !! warn of extrapolation
     if (ne22i > (1.0-rc12)) rne22 = 1.0-rc12
!DON: commented out the warning since it clutters output files A LOT for the CONeNe models
