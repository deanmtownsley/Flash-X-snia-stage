...
!!$  end select
  if (gcell_on_cc(TEMP_VAR)) then
     needEos =.not.(eosMode==MODE_DENS_TEMP .OR. &
  ...
  if (gcell_on_cc(PRES_VAR)) then
     needEos =.not.(eosMode==MODE_DENS_PRES .OR. &
  ...
  if (gcell_on_cc(EION_VAR)) then
     needEos =.not.(eosMode==MODE_DENS_EI .OR. &
  ...
  if(needEos) then
     ...
     !! special stuff for CONe to NSE supernova burning
     ! really there should be some call-outs or overloads here somewhere so that
     ! this function doesn't have to be updated for every EOS variant
     gcell_on_cc(CI_MSCALAR)=.true. ! initial C12 fraction
!don: added ne20
     gcell_on_cc(NE0I_MSCALAR)=.true. ! initial Ne20 fraction
     gcell_on_cc(NE2I_MSCALAR)=.true. ! initial Ne22 fraction
     gcell_on_cc(PHFA_MSCALAR)=.true. ! C12 consumption burning progress variable
     gcell_on_cc(PHAQ_MSCALAR)=.true. ! O16 consumption burning progress variable
     gcell_on_cc(PHQN_MSCALAR)=.true. ! Incomelete Si burning (NS(Q)E) pogress variable
     gcell_on_cc(YE_MSCALAR)=.true. ! electron mole fraction
     gcell_on_cc(DYQN_MSCALAR)=.true. ! scalar for calculating Y_ion for burned material
