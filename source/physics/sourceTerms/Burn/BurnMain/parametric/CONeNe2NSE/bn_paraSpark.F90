! Aaron Jackson 2009 | Josh Martin 2026
!
...
#include "Flash.h"
  ...
  integer :: i
  real :: r, rho, costheta, sintheta, cosphi, sinphi, detD, test_dens, yei
...
!**** This is now hard coded for cartesian 3D
        ! calculate the detonation coordinates
        r = sqrt(x**2 + y**2 + z**2)
        rho = sqrt(x**2 + y**2)
        ! Need to address edge case of detonation being on z-axis
        if (rho > 1.e-10) then
         cosphi = x/rho
         sinphi = y/rho
        else
        ...
        sintheta = rho/r
        r = r + pbIgnDist
        detX = r*sintheta*cosphi
        detY = r*sintheta*sinphi
        detZ = r*costheta
