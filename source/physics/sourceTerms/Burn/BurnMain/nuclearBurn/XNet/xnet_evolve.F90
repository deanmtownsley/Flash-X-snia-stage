    ...
    Do izb = 1, nzbatchmx
      If ( lzactive(izb) ) Then
        If ( t(izb) < tstop(izb) .or. its(izb) > 0 ) Then
          ...
          yout(:) = ystart(:,izb)
          Write(ab_fname,"(4(a,i3.3))") 'ab_fail_',zone_id(1,izb),'_',zone_id(2,izb),'_',zone_id(3,izb),'_',zone_id(4,izb)
          Call write_xnet_inab(ab_fname,ab_fname,nname(1:ny),yout,ierr)
          Write(th_fname,"(4(a,i3.3))") 'th_fail_',zone_id(1,izb),'_',zone_id(2,izb),'_',zone_id(3,izb),'_',zone_id(4,izb)
