  ...
  do while ( t_sub < dt )
     ...
     !   call Grid_getTileIterator(itor, LEAF, level=level, tiling=rt_enableTiling)
        ...
        do while(itor%isValid())
           ...
           !$OMP MAP( to: solnData, lo, hi, u_lo, u_hi, nX )
#elif defined(THORNADO_OACC)
           !$ACC ENTER DATA &
           !$ACC COPYIN( solnData, lo, hi, u_lo, u_hi, nX )
#endif
           ...
           call rt_tm_projection(solnData, nX, lo, hi, u_lo, u_hi)
           call Timers_stop("rt_projection")

           call Timers_start("rt_finalize")
#if defined(THORNADO_OMP_OL)
           !$OMP TARGET EXIT DATA &
           !$OMP MAP( from: solnData ) &
           !$OMP MAP( release: lo, hi, u_lo, u_hi, nX )
#elif defined(THORNADO_OACC)
           !$ACC EXIT DATA &
           !$ACC COPYOUT( solnData ) &
           !$ACC DELETE( lo, hi, u_lo, u_hi, nX )
#endif
