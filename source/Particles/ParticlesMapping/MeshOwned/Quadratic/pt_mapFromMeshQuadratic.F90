...
! x-coord
  ...
  if(ip >= ubound(solnVec,1) .or. ip <= lbound(solnVec,1)) then ! This test relies on REORDERED indices!
...
#ifdef DEBUG_GRIDPARTICLES
  if(jp >= ubound(solnVec,2) .or. jp <= lbound(solnVec,2)) then ! This test relies on REORDERED indices!
     print '(4F8.2)', bndBox(1,1), bndBox(2,1), bndBox(1,2), bndBox(2,2)
     print *, "x, y = ", x, y
     !print *, "newBlockID = ", blockID
     print *, "yp = ", yp
