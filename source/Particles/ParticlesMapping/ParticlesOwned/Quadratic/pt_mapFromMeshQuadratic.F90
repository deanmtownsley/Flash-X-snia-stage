...
#ifdef INDEXREORDER
  integer, parameter :: iX = 1 ! With REORDERED indices, X-index is first.
#else
  integer, parameter :: iX = 2 ! Without reordering, variable index is first.
#endif
...
! x-coord
  ...
  if(ip >= ubound(solnVec,iX) .or. ip <= lbound(solnVec,iX)) then
     print '(4F8.2)', bndBox(1,1), bndBox(2,1), bndBox(1,2), bndBox(2,2)
     print *, "x, y = ", x, y
     !print *, "newBlockID = ", blockID
     print *, "xp = ", xp
     print *, "ip = ", ip, ' not between', lbound(solnVec,iX), ' and', ubound(solnVec,iX)
...
#ifdef DEBUG_GRIDPARTICLES
  if(jp >= ubound(solnVec,iX+1) .or. jp <= lbound(solnVec,iX+1)) then
     print '(4F8.2)', bndBox(1,1), bndBox(2,1), bndBox(1,2), bndBox(2,2)
     print *, "x, y = ", x, y
     print *, "blkLimits:", blkLimits
     !print *, "newBlockID = ", blockID
     print *, "yp = ", yp
     print *, "jp = ", jp, ' not between', lbound(solnVec,iX+1), ' and', ubound(solnVec,iX+1)
     call Driver_abortFlash("we have a problem in pt_mapFromMeshQuadratic")
  end if
#endif
