...
subroutine pt_setMask()
  ...
  use Particles_data, ONLY : pt_gcMaskForAdvance, pt_typeInfo, pt_gcMaskForWrite, pt_meshVar,&
       pt_meshMe, pt_numMappedAttributes, pt_mappedAttributes
  ...
  do i = 1,PT_MAX_ATTRIBUTES
     ...
     if(attributes>0) then
        pt_numMappedAttributes=pt_numMappedAttributes+1
        j=pt_numMappedAttributes
        pt_mappedAttributes(j) = attributes
        call Simulation_mapParticlesVar(pt_mappedAttributes(j),&
             pt_meshVar(PT_VAR,j), pt_meshVar(PT_MAP,j))

#ifdef DEBUG_PARTICLES
        print*,'  attribute #',pt_numMappedAttributes,' =',pt_mappedAttributes(j),'->meshVar',pt_meshVar(PT_VAR,j),pt_meshVar(PT_MAP,j)
#endif
...
99            format(1x,'Particles_init: particle property',i2,' from variable',i2,', map',i2,', attr #',i2,'.')
              print 99, pt_mappedAttributes(j), pt_meshVar(PT_VAR,j), pt_meshVar(PT_MAP,j), j
           end if
           pt_numMappedAttributes = pt_numMappedAttributes - 1 ! ignore this one.
        end select
     else
        if(pt_meshMe==MASTER_PE)then
