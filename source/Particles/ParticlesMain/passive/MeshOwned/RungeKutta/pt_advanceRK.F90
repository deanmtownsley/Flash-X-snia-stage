...
!!  for integration in time. The improved Euler method is one of the family of
...
subroutine pt_advanceRK(dtOld,dtNew, p_beg, p_end, ind)
  ...
  use pt_amrexParticleMod, ONLY : pt_amrexParticle_t, pt_amrexParticlecontainer_t,&
...
#include "Particles.h"
  ...
  type(pt_amrexParticle_t) :: oneParticle
  type(pt_amrexParticle_t), pointer :: particlesSaveVelocity(:)
  type(pt_amrexParticlecontainer_t) :: pcSaveVelocity
  integer :: j
!!------------------------------------------------------------------------------
  ...
  print *, "Debug:", pt_meshMe, "numparticles advanced: ", count
#endif

  call Grid_releaseTileIterator(itor)


  ! Now save the original velocity values in a temporary pt_amrexParticlecontainer_t
  ...
  ! done with this temporary pt_amrexParticlecontainer_t
