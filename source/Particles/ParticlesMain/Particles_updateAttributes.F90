...
#include "Particles.h"
  ...
  integer, dimension(PART_ATTR_DS_SIZE,pt_numMappedAttributes) :: attrib
  ...
  if (pt_numMappedAttributes==0) return
  ...
  do j=1,numGridDataStruct
     ...
     do i = 1,pt_numMappedAttributes
        if(pt_meshVar(PT_MAP,i)==gridStruct(PARTICLE_MAP,j)) then
           numAttrib=numAttrib+1
           attrib(PART_DS_IND,numAttrib)=pt_mappedAttributes(i)
