...
subroutine emit_mcps(dtNew, iSpecies)
  use Particles_data, only : pt_maxPerProc, pt_maxnewnum, pt_numLocal,&
                             pt_meshMe, pt_typeInfo, particles
...
#include "Particles.h"
  ...
  do while(itor%isValid())
    ...
    ! Thermal radiation
    call thermal_emission(tileDesc, solnVec, dtNew, iSpecies,&
    ...
    ! Point emission
    call point_emission(tileDesc, solnVec, dtNew, iSpecies, &
    ...
    ! Face emission
    call face_emission(tileDesc, solnVec, dtNew, iSpecies,&
  ...
  ! if called for all three species dont do above again
  if (iSpecies .eq. THORNADO_NSPECIES) then
    first_call = .false.
  end if
  ...
  if (new_num .GT. 0) then
    ...
    particles(TYPE_PART_PROP, old_pt_numLocal+1:pt_numLocal) = pt_typeInfo(PART_TYPE, iSpecies)
#endif
  ...
  i_old_begin = pt_typeInfo(PART_TYPE_BEGIN,iSpecies)
  i_old_end   = i_old_begin + pt_typeInfo(PART_LOCAL,iSpecies) - 1
...
subroutine thermal_emission(tileDesc, solnVec, dtNew,iSpecies,&
  ...
  use opacity, only : calc_abs_opac, calc_emit_dE, sample_energy_blackbody,&
                      electron_number
  use Driver_interface, only : Driver_abortFlash
  ...
  do k = lo(KAXIS), hi(KAXIS)
    do j = lo(JAXIS), hi(JAXIS)
      do i = lo(IAXIS), hi(IAXIS)
        ...
        call comp_fleck_factor(cellID, solnVec, dtNew, iSpecies)

        do g = 1, size(rt_Emid)
          dE = calc_emit_dE(cellID, solnVec, g, dtNew, dV, iSpecies)

          dE = solnVec(FLEC_VAR,i,j,k) * dE

          ! calculate net energy and lepton number emitted
          ! note that this assumes all particles are emitted at the center of the energy bin.
          dEMIE = -dE / (solnVec(DENS_VAR,i,j,k) * dV)
          dEMIL = dEMIE / rt_Emid(g) * electron_number(iSpecies)
          ...
          if (dE .gt. 0.0d0) then
            ...
            call cellAddVar(solnVec, cellID, EMIL_VAR, dEMIL)
...
subroutine point_emission(tileDesc, solnVec, dtNew, iSpecies,&
                          now_num, now_pos, now_time,&
                          now_energy, now_vel, now_weight, first_call)
  use Particles_data, only : rt_xMax, rt_xMin,&
                              rt_yMax, rt_yMin,&
                              rt_zMax, rt_zMin
...
!  first_call = .false.
  ...
  origin(1) = 0.5*(rt_xMax - rt_xMin)
  origin(2) = 0.5*(rt_yMax - rt_yMin)
  origin(3) = 0.5*(rt_zMax - rt_zMin)
...
subroutine face_emission(tileDesc, solnVec, dtNew, iSpecies, &
           now_num, now_pos, now_time,&
           now_energy, now_vel, now_weight)
  ...
  use opacity, only : calc_face_flux
...
!==========================!
! compute the fleck factor !
!==========================!
subroutine comp_fleck_factor(cellID, solnVec, dt, iSpecies)
  use Particles_data, only : pt_es_alpha, pt_is_eff_scattering
  use Particles_data, only : clight
  use opacity, only : calc_equilibrium_Edens_derivatives, calc_abs_opac, electron_number
  use Particles_data, only : rt_Emid, rt_int4piE2dE, rt_int4piE3dE
  ...
  real :: kp, chip, beta, zeta, gammap, si, denom
  integer :: g
  real, dimension(size(rt_Emid)) :: f0, ka

  if (.not. pt_is_eff_scattering) then

    solnVec(FLEC_VAR, cellID(IAXIS), cellID(JAXIS), cellID(KAXIS)) = 1.0

  else

    ! get the equilibrium distribution and its derivatives
    call calc_equilibrium_Edens_derivatives(cellID, solnVec, iSpecies, f0, beta, zeta)

    ! weighted opacities (Abdikamalov 2012 Eq. 27, 51)
    ! assume that f and ka are constant throughout the energy bin
    do g=1,size(rt_Emid)
      call calc_abs_opac(cellID, solnVec, rt_Emid(g), iSpecies, ka(g))
    end do
    denom = sum(f0 * rt_int4piE3dE)
    if(denom > 0) then
       kp   = sum(ka * f0 * rt_int4piE3dE) / sum(f0 * rt_int4piE3dE) ! 1/cm
       chip = sum(ka * f0 * rt_int4piE2dE) / sum(f0 * rt_int4piE3dE) ! 1/(erg cm)
    else
       kp   = 0
       chip = 0
    end if

    ! IMC gamma (Abdikamalov 2012 Eq. 57)
    si = electron_number(iSpecies)
    gammap = beta*kp + zeta*si*chip

    ! set the fleck factor
    solnVec(FLEC_VAR, cellID(IAXIS), cellID(JAXIS), cellID(KAXIS)) = 1.0d0 / (1.0d0 + (pt_es_alpha * dt * clight * gammap))
