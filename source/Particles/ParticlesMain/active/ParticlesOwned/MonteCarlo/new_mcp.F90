...
#include "Flash.h"
  ...
  if (gr_geometry == SPHERICAL) then
    ...
    newxyz(IAXIS) = bndBox(LOW, IAXIS) + (cellID(IAXIS) - 1 + rand()) * deltaCell(IAXIS)
    if (NDIM >= 2) then
      newxyz(JAXIS) = bndBox(LOW, JAXIS) + (cellID(JAXIS) - 1 + rand()) * deltaCell(JAXIS)

      if (NDIM == 3) then
        newxyz(KAXIS) = bndBox(LOW, KAXIS) + (cellID(KAXIS) - 1 + rand()) * deltaCell(KAXIS)
      end if
    end if
