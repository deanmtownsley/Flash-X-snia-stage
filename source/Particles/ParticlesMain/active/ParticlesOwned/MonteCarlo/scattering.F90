...
subroutine scatter_mcp(solnVec, cellID, particle, iSpecies, &
...
#include "Flash.h"
  ...
  if (is_elastic) then
    ...
    call sample_energy_blackbody(solnVec, cellID, iSpecies, mcp_eps_as)
  end if
  ! Conserve net photon energy if photon energy shifted
