...
!-------------------------------------------------------------------------------
  ...
  call RuntimeParameters_get('gamma', rt_gamma)
  call RuntimeParameters_get('xmin',rt_xMin)
  call RuntimeParameters_get('ymin',rt_yMin)
  call RuntimeParameters_get('zmin',rt_zMin)
  call RuntimeParameters_get('xmax',rt_xMax)
  call RuntimeParameters_get('ymax',rt_yMax)
  call RuntimeParameters_get('zmax',rt_zMax)
  ...
  !========================!
  ! set logarithmically spaced energy grid
  dlogE = -log(pt_energy_min_eV/pt_energy_max_eV) / THORNADO_NE
  do ig = 1,THORNADO_NE
    rt_Ebottom(ig) = pt_energy_max_eV*ev2erg * exp((ig-1)*dlogE + log(pt_energy_min_eV/pt_energy_max_eV))
    rt_Etop(ig)    = pt_energy_max_eV*ev2erg * exp((ig  )*dlogE + log(pt_energy_min_eV/pt_energy_max_eV))
    rt_Emid(ig) = 0.5 * ( rt_Ebottom(ig) + rt_Etop(ig) )
    rt_int4piE2dE(ig) = 4.0/3.0 * PI * ( rt_Etop(ig)**3 - rt_Ebottom(ig)**3)
    rt_int4piE3dE(ig) =           PI * ( rt_Etop(ig)**4 - rt_Ebottom(ig)**4)
  end do

  !======================================!
  ! do whatever the radiation type needs !
  !======================================!
