...
contains
  ...
  ! isActive !
  !==========!
  elemental pure function isActive(trem, iscp)
    implicit none
    real, intent(in) :: trem, iscp
    real :: isActive

    if( (trem .GT. 0.0) .and. (int(iscp) .NE. 1) ) then
       isActive = 1.0
    else
       isActive = 0.0
    endif
  ...
  subroutine transport_mcps(dtNew, iSpecies)
    ...
    use Particles_data, only : pt_typeInfo, pt_maxPerProc, pt_indexList,&
         pt_indexCount, pt_meshMe, pt_numLocal, particles, pt_maxPerProc
    ...
    use opacity, only : calc_abs_opac, calc_sca_opac, &
         calc_abs_opac_vectorized, calc_sca_opac_vectorized
    use scattering, only : scatter_mcp
    use relativity, only : compute_dshift
    use Particles_data, only : clight
...
#include "mpif.h"
    ...
    integer :: i, mcp_begin, mcp_end
    integer, dimension(MAXBLOCKS,NPART_TYPES) :: particlesPerBlk, particlesPerBlk_tmp

    integer :: currentBlk, currentProc
    real, dimension(MDIM) :: currentPos
    integer, allocatable, dimension(:,:) :: cellID
    integer, dimension(MDIM) :: xcellID
    real :: dvol
    ...
    real, dimension(:), allocatable :: k_a, k_s, k_a_cmf, k_s_cmf, dshift, rho, temp, Ye
    ...
    do while (num_done_global /= globalNumParticles)
       ...
       mcp_begin = pt_typeInfo(PART_TYPE_BEGIN,iSpecies)
       mcp_end   = pt_typeInfo(PART_TYPE_BEGIN,iSpecies) + pt_typeInfo(PART_LOCAL,iSpecies) - 1

       ! sort by active status and only loop through active particles
       ! Result is sorted by active_part_prop, then by block ID
       particles(ACTIVE_PART_PROP,mcp_begin:mcp_end) = isActive(particles(TREM_PART_PROP,mcp_begin:mcp_end), particles(ISCP_PART_PROP,mcp_begin:mcp_end))! + 1
       !particlesPerBlk_tmp = particlesPerBlk
       !call Grid_sortParticles(particles(:,mcp_begin:mcp_end), NPART_PROPS, pt_numLocal, 2,&
       !     pt_maxPerProc, particlesPerBlk, BLK_PART_PROP, ACTIVE_PART_PROP)
       !particlesPerBlk = particlesPerBlk_tmp
       !particles(ACTIVE_PART_PROP,mcp_begin:mcp_end) = particles(ACTIVE_PART_PROP,mcp_begin:mcp_end) - 1
       !mcp_begin = mcp_begin + count(particles(ACTIVE_PART_PROP,mcp_begin:mcp_end) .eq. 0.0)


       ! allocate temporary variables
       allocate(k_a(mcp_begin:mcp_end))
       allocate(k_s(mcp_begin:mcp_end))
       allocate(k_a_cmf(mcp_begin:mcp_end))
       allocate(k_s_cmf(mcp_begin:mcp_end))
       allocate(dshift(mcp_begin:mcp_end))
       allocate(rho(mcp_begin:mcp_end))
       allocate(temp(mcp_begin:mcp_end))
       allocate(Ye(mcp_begin:mcp_end))
       allocate(cellID(mcp_begin:mcp_end,MDIM))

       call Timers_start("MCP - whileLoop")

       ! Perform actual radiation transport
       do while (sum(particles(ACTIVE_PART_PROP,mcp_begin:mcp_end)) .gt. 0.0)

          dshift = 1.0d0

!!!!!!!!!!!! NEW DO LOOP THAT ONLY DOES THINGS THAT CONVERTS QUANTITIES THAT DEPEND ON CELLID INTO ARRAYS !!!!!!!!!!!!
          do i = mcp_begin, mcp_end
             if(particles(ACTIVE_PART_PROP,i)==0) then
                cycle
             end if
             ...
             ! coordinate systems.
             if (pt_is_veldp) then
                call compute_dshift(cellID(i,:), solnVec, .false.,&
                     particles(:,i), dshift(i))
             end if

             rho(i) = solnVec(DENS_VAR, cellID(i,IAXIS), cellID(i,JAXIS), cellID(i,KAXIS))
             temp(i) = solnVec(TEMP_VAR, cellID(i,IAXIS), cellID(i,JAXIS), cellID(i,KAXIS))
#ifdef YE_MSCALAR
             Ye(i) = solnVec(YE_MSCALAR, cellID(i,IAXIS), cellID(i,JAXIS), cellID(i,KAXIS))
#endif
             ! evaluate the fleck factor
             fleck = solnVec(FLEC_VAR, cellID(i,IAXIS), cellID(i,JAXIS), cellID(i,KAXIS))
             if (.not. pt_is_eff_scattering) fleck = 1.0d0

             ! calculate opacities
             call calc_abs_opac(cellID(i,:), solnVec, particles(ENER_PART_PROP,i), iSpecies, k_a_cmf(i))
             call calc_sca_opac(cellID(i,:), solnVec, particles(ENER_PART_PROP,i), iSpecies, k_s_cmf(i))
             k_a(i) = k_a_cmf(i)*dshift(i)
             k_s(i) = k_s_cmf(i)*dshift(i)
             ...
             !  print *, "fate,ka,ks", mcp_fate, k_a(i), k_s(i)
             !  print *, "fleck", fleck
             !  print *, "vel*", particles(VELX_PART_PROP:VELZ_PART_PROP,i)
             !  print *, "cellID(i,:)", cellID(i,:)
             ...
             if (numpass > pt_max_rt_iterations) then
                ...
                print *, "vel", particles(VELX_PART_PROP:VELZ_PART_PROP,i)
                print *, "now cellID", cellID(i,:)
                print *, "target xcellID", xcellID
                !print *, "ka/ks", k_a(i), k_s(i), fleck
                !print *, "dvol", dvol
                !print *, "rho", solnVec(DENS_VAR, cellID(i,1), cellID(i,2), cellID(i,3))
                !print *, "temp", solnVec(TEMP_VAR, cellID(i,1), cellID(i,2), cellID(i,3))
             ...
             case (pt_STAY_ID)
                ...
                call deposit_energy_momentum(solnVec, cellID(i,:), particles(:,i),&
                     mcp_fate, dt, dtNew,&
                     fleck, k_a_cmf(i), k_s_cmf(i), dvol, iSpecies)
             ...
             case (pt_SCAT_ID:pt_PION_ESCAT_ID) ! Physical and effective scattering
                ...
                if (mcp_fate == pt_SCAT_ID) then
                   call scatter_mcp(solnVec, cellID(i,:), particles(:,i), iSpecies,&
             ...
             case (pt_CROSS_ID)
                ...
                !call sanitize_phi_periodic_BCs(cellID(i,:), xcellID, &
                !                           bndBox, deltaCell, particles(:,i))

                ! Make sure the xcellID is updated.
                ! For boundary crossing, 4/13 are corrected to 12/5.
                !call sanitize_xblock_cellID(cellID(i,:), xcellID)
                ...
                if (isoutside) then
                   ...
                   if (neghID(PROCNO) /= currentProc) then
                      ...
                      call sanitize_cart_periodic_BCs(cellID(i,:), xcellID, &
                           bndBox, deltaCell, particles(:,i))
                ...
                if ((.NOT. is_crossed) .and. (.not. is_crossproc)) then
                   ...
                   print *, "bndbox-x", bndBox(LOW,1) + deltaCell(1)*(cellID(i,1) - NGUARD - 1),&
                        bndBox(LOW,1) + deltaCell(1)*(cellID(i,1) - NGUARD) 
                   print *, "bndbox-y", bndBox(LOW,2) + deltaCell(2)*(cellID(i,2) - NGUARD - 1),&
                        bndBox(LOW,2) + deltaCell(2)*(cellID(i,2) - NGUARD) 
                   print *, "bndbox-z", bndBox(LOW,3) + deltaCell(3)*(cellID(i,3) - NGUARD - 1),&
                        bndBox(LOW,3) + deltaCell(3)*(cellID(i,3) - NGUARD) 
                   ...
                   call calc_distance_to_theta_wall(bndBox, deltaCell,&
                        particles(:, i), cellID(i,:), theta_dist, theta_dir, .true.)
                   call calc_distance_to_phi_wall(bndBox, deltaCell,&
                        particles(:, i), cellID(i,:), phi_dist, phi_dir, .true.)
                   call distance_to_closest_wall(bndBox, deltaCell, particles(:, i), cellID(i,:),&
             ...
             particles(ACTIVE_PART_PROP,i) = isActive(particles(TREM_PART_PROP, i), particles(ISCP_PART_PROP, i))
       ...
       num_sinks = sum(pt_typeInfo(PART_LOCAL,:)) - pt_typeInfo(PART_LOCAL,iSpecies)
       ! Offsetting with the number of active particles
       ! RHS is total number - photons, i.e. active particles
       num_done_local = num_sinks 
       do i = mcp_begin, mcp_end
          if(particles(TREM_PART_PROP,i) <= 0.0d0) then
             num_done_local = num_done_local + 1
          endif
       enddo
       ! include the particles that we skipped over in the entire loop
       num_done_local = num_done_local + (mcp_begin - pt_typeInfo(PART_TYPE_BEGIN,iSpecies))
...
#include "Flash.h"
  ...
  subroutine sanitize_phi_periodic_BCs(cellID, xcellID,&
    ...
    use Particles_data, only : pt_smlpush
    use Grid_data, only: gr_geometry
    use Particles_data, only : rt_zMin, rt_zMax
    use Grid_iterator, ONLY : Grid_iterator_t
    use Grid_tile,        ONLY : Grid_tile_t
    implicit none
#include "constants.h"
#include "Flash.h"
       ...
       if (delta_cellID(ii) == 1) then
          ...
          if (ii == KAXIS) then
             ...
             if ((gr_geometry == SPHERICAL) .and.&
                ...
                !  now_pos(ii) = now_pos(ii) + 2.0*PI
                !end if
                if (now_pos(ii) > rt_zMax) then
                   now_pos(ii) = now_pos(ii) - rt_zMax
                end if
  ...
  subroutine sanitize_cart_periodic_BCs(cellID, xcellID,&
    ...
    use Particles_data, only : pt_smlpush
    use Grid_data, only: gr_geometry
    use Particles_data, only : rt_xMin, rt_xMax,&
         rt_yMin, rt_yMax,&
         rt_zMin, rt_zMax
    use Grid_iterator, ONLY : Grid_iterator_t
    use Grid_tile,        ONLY : Grid_tile_t
...
#include "Flash.h"
    ...
    domain_min(1) = rt_xMin
    domain_min(2) = rt_yMin
    domain_min(3) = rt_zMin
    domain_max(1) = rt_xMax
    domain_max(2) = rt_yMax
    domain_max(3) = rt_zMax
...
#include "Particles.h"
  ...
  subroutine deposit_energy_momentum(solnVec, cellID, particle,&
    ...
    use Driver_interface, only : Driver_abortFlash
    use opacity, only : electron_number
    implicit none 
#include "constants.h"
#include "Flash.h"
    ...
    integer, dimension(MDIM), intent(in) :: cellID
    real, dimension(NPART_PROPS), intent(inout) :: particle
    integer, intent(in) :: mcp_fate, iSpecies
    ...
    real :: mcp_eps, mcp_energy, avg_weight
    real :: delta_e, delta_l, rho, u_rad
    real :: radflux, radflux_a, radflux_s
    real :: theta, phi
    real, dimension(MDIM) :: x_hat, y_hat, z_hat
    real, dimension(MDIM) :: n_hat
    real :: a_x, a_y, a_z, aa_x, aa_y, aa_z, as_x, as_y, as_z
