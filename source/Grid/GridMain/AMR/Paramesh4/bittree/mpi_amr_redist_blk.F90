...
!-----Local variables. and arrays.
      integer :: lev, lcoord(3),proc,locblk
...
!-----compute old_loc ((-1,-1) if newchild)
      old_loc(:,:) = -1
      do i=1,new_lnblocks
...
!-----With bittree, new_loc = -1 for blocks being derefined.
      do lb=1,lnblocks_old
        if(new_loc(1,lb).lt.0) then
          moved(lb)=.TRUE.
          free(lb)=.TRUE.
        end if
      end do
