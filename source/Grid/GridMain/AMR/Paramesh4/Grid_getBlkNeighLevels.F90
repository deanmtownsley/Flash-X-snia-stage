...
#if NDIM > 1
  if (present(trackBdry)) then
     if (trackBdry) then
        do k = -K3D,K3D
           do j = -1,1
              do i = -1,1
                 if (levels(i,j,k) == myRefine) then
                    if (surr_blks(1,i+2,j+2,k+1+K3D,blockID) <= PARAMESH_PHYSICAL_BOUNDARY) then
                       if (abs(i)+abs(j)+abs(k) == 2) then
                          nearestMin = &
                               minval(levels(max(-1,i-1):min(1,i+1), &
                                             max(-1,j-1):min(1,j+1), &
                                             max(-K3D,k-1):min(K3D,j+1)))
                          nearestMax = &
                               maxval(levels(max(-1,i-1):min(1,i+1), &
                                             max(-1,j-1):min(1,j+1), &
                                             max(-K3D,k-1):min(K3D,j+1)))
                          if (nearestMin < myRefine) then
                             levels(i,j,k) = nearestMin
                          else
                             levels(i,j,k) = nearestMax
                          end if
                       end if
                    end if
                 end if
...
#  if NDIM == 3
        do k = -K3D,K3D
           do j = -1,1
              do i = -1,1
                 if (levels(i,j,k) == myRefine) then
                    if (surr_blks(1,i+2,j+2,k+2,blockID) <= PARAMESH_PHYSICAL_BOUNDARY) then
                       if (abs(i)+abs(j)+abs(k) == 3) then
                          nearestMin = &
                               minval(levels(max(-1,i-1):min(1,i+1), &
                                             max(-1,j-1):min(1,j+1), &
                                             max(-1,k-1):min(1,j+1)))
